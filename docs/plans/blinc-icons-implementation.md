...
### 1.2 Binary Size Strategy: Rely on DCE

Instead of feature flags, we leverage **Rust's Dead Code Elimination (DCE)**:

1. Generate all ~1000+ icons as `pub const` in a single module
2. Unused constants are automatically stripped by the compiler
3. Only icons actually referenced in code end up in the binary

**Why this works:**

- `const` values are inlined at compile time
- If a const is never referenced, LLVM eliminates it during optimization
- No runtime overhead, no complex feature combinations
- Users get tree-shaking for free

```rust
// Generated: all icons as const
pub mod icons {
    pub const ARROW_RIGHT: &str = r#"<path d="M5 12h14m-7-7 7 7-7 7"/>"#;
    pub const CHECK: &str = r#"<path d="M20 6 9 17l-5-5"/>"#;
    // ... 1000+ more
}

// Only ARROW_RIGHT ends up in binary if CHECK is never used
use blinc_icons::icons::ARROW_RIGHT;
```

### 1.3 Naming Convention

SVG filenames → Rust const names:

- `arrow-right.svg` → `ARROW_RIGHT`
- `chevron-down.svg` → `CHEVRON_DOWN`
- `alert-circle.svg` → `ALERT_CIRCLE`
...
[dependencies]
# No runtime dependencies - pure static data
# DCE handles binary size optimization automatically
```

### 2.2 Generated Icon Constants

Each icon becomes a simple `const` with its SVG path data. DCE eliminates unused ones.

```rust
// src/icons.rs (generated by build.rs)
...
// Or use cn::icon() which wraps this (no explicit .build() needed)
...
/// Icon builder - implements ElementBuilder directly for implicit conversion
...
// ElementBuilder implementation - no explicit .build() call needed
impl ElementBuilder for IconBuilder {
    ...
    // ... other ElementBuilder trait methods delegate to inner
}
