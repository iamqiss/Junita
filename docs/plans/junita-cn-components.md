# junita_cn Component Library Plan

A comprehensive shadcn-inspired component library built on `junita_layout` primitives.

## Architecture

```
junita_theme      - Design tokens (colors, spacing, radii, shadows, typography)
     ↓
junita_layout     - Primitives (div, text, scroll, motion, stateful, etc.)
     ↓
junita_cn         - Styled components (Button, Card, Dialog, etc.)
```
...
Icon system with Lucide icons. See [junita-icons-implementation.md](./junita-icons-implementation.md) for detailed plan.
...
```
junita_icons/              # Icon library crate
├── Cargo.toml
├── src/
...
fn main() {
    ...
    junita_icons_codegen::generate(icons_dir, output);
}
```
...
use junita_icons::{icon, IconSize};

fn toolbar(ctx: &impl JunitaContext) -> impl ElementBuilder {
...
2. **Leverage Primitives**: Build on `junita_layout` primitives, don't recreate
...
- All components use theme tokens from junita_theme
