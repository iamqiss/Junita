...
//!
//! Creates opinionated workspace structure for Junita projects:
...
use crate::config::JunitaProject;

/// Create a new Junita project with full workspace structure
pub fn create_project(path: &Path, name: &str, template: &str, org: &str) -> Result<()> {
    ...
    // Create .junitaproj
    let project = JunitaProject::new(name).with_all_platforms(name, org);
    fs::write(path.join(".junitaproj"), project.to_toml()?)?;
    ...
    fs::write(path.join("src/main.junita"), main_content)?;
...
# Web (WASM)
junita build --target wasm --release
```
...
"#
    ...
    fs::write(
        ...
        r#"// Top-level build file for Junita Android project
plugins {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    <application
        ...
        android:theme="@style/Theme.Junita">
...
/**
 * Main entry point for the Junita Android application.
 * The Junita runtime will initialize and render the UI here.
 */
...
"#
    ...
    fs::write(
        ...
        r#"<?xml version="1.0" encoding="utf-8"?>
<resources>
    <style name="Theme.Junita" parent="android:Theme.Material.Light.NoActionBar">
...
"#,
    ...
    fs::write(
        ...
        r#"# Junita Android project properties
...
<component type="desktop-application">
    ...
    <name>{name}</name>
    <summary>A Junita application</summary>
    <metadata_license>CC0-1.0</metadata_license>
    <project_license>MIT</project_license>
    <description>
        <p>
            {name} is a cross-platform application built with Junita.
        </p>
    </description>
...
<head>
    ...
    <meta name="description" content="{name} - A Junita Application">
    <title>{name}</title>
...
<body>
    ...
    <canvas id="junita-canvas"></canvas>
...
"##
    ...
    "name": "{name}",
    "short_name": "{name}",
    "description": "A Junita Application",
    "start_url": "/",
    "display": "standalone",
    "orientation": "any",
...
Configure the preferred backend in `.junitaproj`:
...
A Junita ZRTL plugin.

## Building

### Dynamic (.zrtl)
```bash
junita plugin build --mode dynamic
```

### Static
```bash
junita plugin build --mode static
```

## Usage

Import in your Junita application:
...
fn template_default(name: &str) -> String {
    format!(
        r#"// {name} - Junita Application
//
// A simple Junita application with reactive state and animations.
...
fn template_minimal(name: &str) -> String {
    format!(
        r#"// {name} - Minimal Junita Application

@widget App {{
    @render {{
        Text {{
            content: "Hello, Junita!"
        }}
    }}
}}
"#
...
/// Create a new Rust-first Junita project
///
/// This creates a native Rust project with Cargo.toml instead of .junita DSL files.
...
pub fn create_rust_project(path: &Path, name: &str, org: &str) -> Result<()> {
    ...
    // Get junita workspace path (relative to the generated project)
    let junita_path = std::env::var("JUNITA_PATH").unwrap_or_else(|_| {
        // Try to find the junita workspace relative to the CLI binary
    ...
    // Create directory structure
    fs::create_dir_all(path.join("src"))?;
    fs::create_dir_all(path.join("platforms/android/app/src/main/kotlin/com/junita"))?;
    fs::create_dir_all(path.join("platforms/ios/JunitaApp"))?;
...
[dependencies]
junita_app = {{ path = "{junita_path}/crates/junita_app" }}
junita_core = {{ path = "{junita_path}/crates/junita_core" }}
junita_layout = {{ path = "{junita_path}/crates/junita_layout" }}
tracing = "0.1"
tracing-subscriber = "0.3"

[target.'cfg(target_os = "android")'.dependencies]
junita_platform_android = {{ path = "{junita_path}/extensions/junita_platform_android" }}
android-activity = {{ version = "0.6", features = ["native-activity"] }}
log = "0.4"
android_logger = "0.14"

[target.'cfg(target_os = "ios")'.dependencies]
junita_platform_ios = {{ path = "{junita_path}/extensions/junita_platform_ios" }}

[target.'cfg(any(target_os = "windows", target_os = "macos", target_os = "linux"))'.dependencies]
junita_platform_desktop = {{ path = "{junita_path}/extensions/junita_platform_desktop" }}
...
"#
    ...
    // Create junita.toml
    fs::write(
        path.join("junita.toml"),
        format!(
            r#"# Junita Project Configuration (Rust)
# Generated by: junita new --rust
...
"#
    ...
    // MainActivity.kt
    let kotlin_path = android_path.join("app/src/main/kotlin/com/junita");
...
fn create_rust_ios_files(path: &Path, name: &str, package_name: &str, org: &str) -> Result<()> {
    let ios_path = path.join("platforms/ios");
    let app_path = ios_path.join("JunitaApp");
    let xcodeproj = ios_path.join("JunitaApp.xcodeproj");
...
"#,
    ...
    // JunitaMetalView.swift
    fs::write(
        app_path.join("JunitaMetalView.swift"),
        r#"import UIKit
import Metal
import QuartzCore

class JunitaMetalView: UIView {
...
"#,
    ...
    fs::write(
        app_path.join("JunitaViewController.swift"),
        r#"import UIKit
import MetalKit

class JunitaViewController: UIViewController {
    private var metalView: JunitaMetalView!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        metalView = JunitaMetalView(frame: view.bounds)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let ctx = renderContext { junita_set_focused(ctx, true) }
    }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if let ctx = renderContext { junita_set_focused(ctx, false) }
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ctx = renderContext { junita_update_size(ctx, width, height, Double(scale)) }
        if let gpu = gpuRenderer { junita_gpu_resize(gpu, width, height) }
    }

    deinit {
        displayLink?.invalidate()
        if let gpu = gpuRenderer { junita_destroy_gpu(gpu) }
        if let ctx = renderContext { junita_destroy_context(ctx) }
    }

    private func initializeJunita() {
        let scale = UIScreen.main.scale
        let width = UInt32(view.bounds.width * scale)
        let height = UInt32(view.bounds.height * scale)
        guard let ctx = junita_create_context(width, height, Double(scale)) else {
            print("Error: Failed to create Junita render context")
            return
        }
        renderContext = ctx
        let metalLayer = metalView.metalLayer
        guard let gpu = junita_init_gpu(ctx, Unmanaged.passUnretained(metalLayer).toOpaque(), width, height) else {
            print("Error: Failed to initialize Junita GPU renderer")
            return
        }
        gpuRenderer = gpu
        print("Junita initialized: \(width)x\(height) @ \(scale)x")
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            junita_handle_touch(ctx, getTouchId(for: touch), Float(point.x), Float(point.y), 0)
        }
    }
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            let point = touch.location(in: view)
            junita_handle_touch(ctx, getTouchId(for: touch), Float(point.x), Float(point.y), 1)
        }
    }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            let point = touch.location(in: view)
            junita_handle_touch(ctx, getTouchId(for: touch), Float(point.x), Float(point.y), 2)
    ...
    override func touchesCancelled(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let ctx = renderContext else { return }
        for touch in touches {
            junita_handle_touch(ctx, getTouchId(for: touch), Float(touch.location(in: view).x), Float(touch.location(in: view).y), 3)
            removeTouchId(for: touch)
        }
    }
...
"#,
    ...
    // Junita-Bridging-Header.h
    fs::write(
        app_path.join("Junita-Bridging-Header.h"),
        format!(
            r#"// Junita-Bridging-Header.h - Rust FFI declarations for {name}

#ifndef Junita_Bridging_Header_h
#define Junita_Bridging_Header_h
...
// Context Lifecycle
IOSRenderContext* junita_create_context(uint32_t width, uint32_t height, double scale_factor);
void junita_destroy_context(IOSRenderContext* ctx);

// Rendering
bool junita_needs_render(IOSRenderContext* ctx);
void junita_set_ui_builder(UIBuilderFn builder);
void junita_build_frame(IOSRenderContext* ctx);
bool junita_tick_animations(IOSRenderContext* ctx);

// Window Size
void junita_update_size(IOSRenderContext* ctx, uint32_t width, uint32_t height, double scale_factor);
float junita_get_width(IOSRenderContext* ctx);
float junita_get_height(IOSRenderContext* ctx);
uint32_t junita_get_physical_width(IOSRenderContext* ctx);
uint32_t junita_get_physical_height(IOSRenderContext* ctx);
double junita_get_scale_factor(IOSRenderContext* ctx);

// Input Events
void junita_handle_touch(IOSRenderContext* ctx, uint64_t touch_id, float x, float y, int32_t phase);
void junita_set_focused(IOSRenderContext* ctx, bool focused);

// State Management
void junita_mark_dirty(IOSRenderContext* ctx);
void junita_clear_dirty(IOSRenderContext* ctx);
WindowedContext* junita_get_windowed_context(IOSRenderContext* ctx);

// GPU Rendering
IOSGpuRenderer* junita_init_gpu(IOSRenderContext* ctx, void* metal_layer, uint32_t width, uint32_t height);
void junita_gpu_resize(IOSGpuRenderer* gpu, uint32_t width, uint32_t height);
bool junita_render_frame(IOSGpuRenderer* gpu);
void junita_destroy_gpu(IOSGpuRenderer* gpu);
...
/* Begin PBXBuildFile section */
		...
		A1000002 /* JunitaViewController.swift in Sources */ = {{isa = PBXBuildFile; fileRef = A2000002; }};
		A1000003 /* JunitaMetalView.swift in Sources */ = {{isa = PBXBuildFile; fileRef = A2000003; }};
...
/* Begin PBXFileReference section */
		...
		A2000002 /* JunitaViewController.swift */ = {{isa = PBXFileReference; lastKnownFileType = sourcecode.swift; path = JunitaViewController.swift; sourceTree = "<group>"; }};
		A2000003 /* JunitaMetalView.swift */ = {{isa = PBXFileReference; lastKnownFileType = sourcecode.swift; path = JunitaMetalView.swift; sourceTree = "<group>"; }};
		...
		A2000009 /* Junita-Bridging-Header.h */ = {{isa = PBXFileReference; lastKnownFileType = sourcecode.c.h; path = "Junita-Bridging-Header.h"; sourceTree = "<group>"; }};
		A3000001 /* JunitaApp.app */ = {{isa = PBXFileReference; explicitFileType = wrapper.application; includeInIndex = 0; path = JunitaApp.app; sourceTree = BUILT_PRODUCTS_DIR; }};
...
/* Begin XCBuildConfiguration section */
		A9000001 /* Debug */ = {{
			...
			buildSettings = {{
				...
				INFOPLIST_FILE = JunitaApp/Info.plist;
				...
				SWIFT_OBJC_BRIDGING_HEADER = "JunitaApp/Junita-Bridging-Header.h";
				SWIFT_VERSION = 5.0;
...
/* Begin XCConfigurationList section */
		A8000001 /* Build configuration list for PBXNativeTarget "JunitaApp" */ = {{
			isa = XCConfigurationList;
			buildConfigurations = (A9000001, A9000002);
			defaultConfigurationIsVisible = 0;
			defaultConfigurationName = Release;
		}};
		A8000002 /* Build configuration list for PBXProject "JunitaApp" */ = {{
			isa = XCConfigurationList;
...
echo ""
echo "Open JunitaApp.xcodeproj in Xcode to build and run."
"#
        ),
    )?;
