...
pub fn run_doctor() -> Vec<CheckCategory> {
    ...
    categories.push(check_junita_ecosystem());
...
/// Check Junita runtime and compiler ecosystem
fn check_junita_ecosystem() -> CheckCategory {
    let mut cat = CheckCategory::new("Junita Ecosystem");
    ...
    // Check junita CLI version
    cat.add(CheckResult::ok(
        "Junita CLI",
        &format!("v{} ({})", version, git_hash),
    ));

    // Junita Runtime (same repo, same version)
    cat.add(CheckResult::ok(
        "Junita Runtime",
        &format!("v{} ({})", version, git_hash),
    ));
    ...
    // Check for junita.toml in current directory (optional)
    let cwd = std::env::current_dir();
    if let Ok(dir) = cwd {
        let config_path = dir.join("junita.toml");
        if config_path.exists() {
            cat.add(CheckResult::ok(
                "Project config",
                &format!("junita.toml found in {}", dir.display()),
            ));
        } else {
            cat.add(CheckResult::not_applicable(
                "Project config",
                "no junita.toml in current directory",
            ));
        }
    }
