//! Junita configuration file handling
//!
//! Junita uses two configuration files:
//! - `.junitaproj` - Project configuration (metadata, dependencies, targets)
//! - `junita.toml` - Workspace configuration (build settings, dev server)
...
/// Project configuration stored in .junitaproj
#[derive(Debug, Deserialize, Serialize)]
pub struct JunitaProject {
...
impl JunitaProject {
    /// Load project configuration from .junitaproj
    pub fn load_from_dir(path: &Path) -> Result<Self> {
        let config_path = path.join(".junitaproj");

        if !config_path.exists() {
            anyhow::bail!(
                "No .junitaproj found in {}. Run `junita init` to create one.",
                path.display()
            );
        }
        ...
        let config: JunitaProject = toml::from_str(&content)
...
// =============================================================================
// junita.toml - Workspace Configuration (Legacy/Backward Compatibility)
// =============================================================================

/// Workspace-level Junita configuration (junita.toml)
...
impl JunitaConfig {
    ...
    /// Checks for .junitaproj first, falls back to junita.toml
    pub fn load_from_dir(path: &Path) -> Result<Self> {
        let junitaproj_path = path.join(".junitaproj");
        let junita_toml_path = path.join("junita.toml");

        // Try .junitaproj first (new format)
        if junitaproj_path.exists() {
            let project = JunitaProject::load_from_dir(path)?;
            return Ok(Self::from_project(&project));
        }

        // Fall back to junita.toml (legacy format)
        if junita_toml_path.exists() {
            let content = fs::read_to_string(&junita_toml_path)
                .with_context(|| format!("Failed to read {}", junita_toml_path.display()))?;

            let config: JunitaConfig = toml::from_str(&content)
                .with_context(|| format!("Failed to parse {}", junita_toml_path.display()))?;

            return Ok(config);
        }

        anyhow::bail!(
            "No .junitaproj or junita.toml found in {}. Run `junita init` to create one.",
            path.display()
        );
    }

    /// Convert from JunitaProject to JunitaConfig
    fn from_project(project: &JunitaProject) -> Self {
        Self {
