//! # Junita Component Library (junita_cn)
//!
//! A shadcn-inspired component library built on `junita_layout` primitives.
//!
//! ## Philosophy
//!
//! Like shadcn/ui builds styled components on top of Radix UI primitives,
//! `junita_cn` builds themed, accessible components on top of `junita_layout`.
//!
//! - **Primitives**: `junita_layout` provides low-level building blocks (div, text, scroll, etc.)
//! - **Theme Tokens**: `junita_theme` provides design tokens (colors, spacing, radii, shadows)
//! - **Components**: `junita_cn` provides styled components that use theme tokens
//!
//! ## Example
//!
//! ```ignore
...
// Re-export InstanceKey from junita_layout (the canonical location)
pub use junita_layout::InstanceKey;
...
pub mod prelude {
    ...
    pub use junita_core::State;
    // Re-export SchedulerHandle for slider/switch usage
    pub use junita_animation::SchedulerHandle;
    // Re-export text_area_state for textarea usage
    pub use junita_layout::widgets::text_area::{text_area_state, SharedTextAreaState};
    // Re-export commonly needed theme types
    pub use junita_theme::{ColorToken, RadiusToken, ShadowToken, SpacingToken, ThemeState};
    // Re-export icons module for easy access
    pub use junita_icons::icons;
}
