...
use junita_layout::div::ElementBuilder;
use junita_layout::element::CursorStyle;
use junita_layout::prelude::*;
use junita_layout::stateful::{stateful_with_key, use_shared_state, ButtonState, SharedState};
...
struct ButtonConfig {
    ...
    on_click: Option<Arc<dyn Fn(&junita_layout::event_handler::EventContext) + Send + Sync>>,
}
    ...
    fn from_config(instance_key: &str, config: ButtonConfig) -> Self {
        ...
        let mut stateful = stateful_with_key::<ButtonState>(&state_key)
            ...
            .on_state(move |ctx| {
                ...
                let mut content = junita_layout::div::div().flex_row().items_center().gap(6.0);
                ...
                if let Some(ref icon_str) = icon {
                    // Convert path data to full SVG using junita_icons
                    let icon_size = font_size + 2.0; // Slightly larger for visibility
                    let svg_str = junita_icons::to_svg(icon_str, icon_size);
                ...
                let mut visual = div()
                    ...
                    .w_fit()
                    .bg(bg)
                    .transform(junita_core::Transform::scale(scale, scale));
...
impl ElementBuilder for Button {
    ...
    fn element_type_id(&self) -> junita_layout::div::ElementTypeId {
...
impl ButtonBuilder {
    ...
    where
        F: Fn(&junita_layout::event_handler::EventContext) + Send + Sync + 'static,
    {
