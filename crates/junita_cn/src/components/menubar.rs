...
use junita_layout::widgets::hr::hr_with_bg;
use junita_layout::widgets::overlay::{OverlayAnimation, OverlayHandle, OverlayManagerExt};
...
impl MenubarBuilder {
    ...
    fn build_component(&self) -> Menubar {
        ...
        let active_menu: State<Option<usize>> =
            JunitaContextState::get().use_state_keyed(self.key.get(), || None);

        // State for overlay handle
        let overlay_handle_state: State<Option<u64>> =
            JunitaContextState::get().use_state_keyed(&self.key.derive("handle"), || None);
...
fn show_menubar_hover_dropdown(
    ...
    let handle = mgr
        .hover_card()
        .at(x, y)
        .anchor_direction(junita_layout::widgets::overlay::AnchorDirection::Bottom)
        .animation(OverlayAnimation::none()) // Instant show/hide
        .dismiss_on_escape(true)
        .on_close(move || {
            active_menu_for_close.set(None);
...
fn build_menubar_hover_dropdown_content(
    ...
    let submenu_handle: State<Option<u64>> =
        JunitaContextState::get().use_state_keyed(&format!("{}_submenu", key), || None);
...
fn show_menubar_submenu(
    ...
    let handle = mgr
        .hover_card()
        .at(x, y)
        .anchor_direction(junita_layout::widgets::overlay::AnchorDirection::Right)
        .animation(OverlayAnimation::none()) // Instant show/hide
        .dismiss_on_escape(true)
        .on_close(move || {
...
fn build_menubar_submenu_content(
    ...
    let nested_submenu_handle: State<Option<u64>> =
        JunitaContextState::get().use_state_keyed(&format!("{}_nested", key), || None);
