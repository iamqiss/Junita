...
//!
//! Renders Lucide icons from junita_icons with theming support.
//!
//! # Example
//!
//! ```ignore
...
impl Icon {
    ...
    fn from_config(config: &IconConfig) -> Self {
        ...
        let stroke_width = config.stroke_width.unwrap_or(junita_icons::STROKE_WIDTH);
        ...
        // Generate SVG string
        let svg_str = junita_icons::to_svg_with_stroke(config.path_data, size, stroke_width);
...
impl IconBuilder {
    ...
    ///
    /// # Arguments
    /// * `path_data` - SVG inner content from `junita_icons::icons::*`
