...
use junita_animation::{get_scheduler, AnimatedValue, SchedulerHandle, SpringConfig};
use junita_core::{Color, State, Transform};
use junita_layout::div::ElementTypeId;
use junita_layout::element::RenderProps;
use junita_layout::motion::SharedAnimatedValue;
...
impl Switch {
    ...
    fn with_config(config: SwitchConfig) -> Self {
        ...
        let mut switch = div()
            ...
            .relative()
            .items_center()
            .padding_x(junita_layout::units::px(padding))
            // Background layers
            .child(off_layer)
            .child(on_layer)
            // Animated thumb
