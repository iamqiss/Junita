...
use junita_core::State;
use junita_layout::div::ElementTypeId;
use junita_layout::element::{CursorStyle, RenderProps};
use junita_layout::motion::motion_derived;
use junita_layout::overlay_state::get_overlay_manager;
use junita_layout::prelude::*;
use junita_layout::stateful::{stateful_with_key, ButtonState};
use junita_layout::tree::{LayoutNodeId, LayoutTree};
use junita_layout::widgets::overlay::{OverlayHandle, OverlayManagerExt};
use junita_layout::widgets::scroll::scroll;
use junita_layout::widgets::text_input::SharedTextInputData;
use junita_theme::{ColorToken, RadiusToken, SpacingToken, ThemeState};

use super::label::{label, LabelSize};
use junita_layout::InstanceKey;
...
impl Combobox {
    ...
    fn from_config(instance_key: &str, config: ComboboxConfig) -> Self {
        ...
        let open_state = JunitaContextState::get().use_state_keyed(&open_key, || false);
        ...
        let overlay_handle_state: State<Option<u64>> =
            JunitaContextState::get().use_state_keyed(&handle_key, || None);
        ...
        let search_input_data: SharedTextInputData = JunitaContextState::get()
            .use_state_keyed(&search_key, || {
                junita_layout::widgets::text_input::text_input_data()
            })
            .get();
        ...
        let search_query_state: State<String> =
            JunitaContextState::get().use_state_keyed(&search_query_key, || String::new());
...
impl ElementBuilder for ComboboxBuilder {
    ...
    fn event_handlers(&self) -> Option<&junita_layout::event_handler::EventHandlers> {
...
fn build_dropdown_content(
    ...
    padding: f32,
    radius: f32,
    bg: junita_core::Color,
    border: junita_core::Color,
    border_focus: junita_core::Color,
    text_color: junita_core::Color,
    text_tertiary: junita_core::Color,
    surface_elevated: junita_core::Color,
    allow_custom: bool,
    placeholder: &Option<String>,
) -> Div {
    let theme = ThemeState::get();
    ...
    let search_input = junita_layout::widgets::text_input::text_input(search_data)
        .w_full()
        .h(height)
        .text_size(font_size)
