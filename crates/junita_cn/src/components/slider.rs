...
use junita_layout::stateful::{stateful_with_key, NoState, StateTransitions};
...
/// JunitaComponent for slider state and animations
...
impl Slider {
    ...
    fn with_config(key: InstanceKey, config: SliderConfig) -> Self {
        ...
        // Get PERSISTED state from context using JunitaComponent macro
        ...
        let inner = if config.label.is_some() || config.show_value {
            let spacing = theme.spacing_value(junita_theme::SpacingToken::Space2);
...
/// Uses JunitaComponent macro for type-safe state management.
///
