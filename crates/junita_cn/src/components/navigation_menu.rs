...
impl NavigationMenu {
    fn from_builder(builder: &NavigationMenuBuilder) -> Self {
        ...
        let active_menu: State<Option<usize>> =
            JunitaContextState::get().use_state_keyed(&format!("{}_active", key_base), || None);
        // Track the current overlay handle
        let overlay_handle_state: State<Option<u64>> =
            JunitaContextState::get().use_state_keyed(&format!("{}_handle", key_base), || None);
                            ...
                            let link_item = stateful_with_key::<ButtonState>(&format!("{}_btn", item_key))
                                .on_state(move |ctx| {
                                    ...
                                    let (bg, text_color) = match state {
                                        ...
                                        _ => (junita_core::Color::TRANSPARENT, text_secondary),
                                    };
                            ...
                            let trigger_item = stateful_with_key::<ButtonState>(&format!("{}_btn", item_key))
                                ...
                                .on_state(move |ctx| {
                                        ...
                                        || state == ButtonState::Pressed
                                    {
                                        ...
                                        (junita_core::Color::TRANSPARENT, text_secondary)
                                    };
                                ...
                                .on_hover_enter(move |ctx| {
                                    ...
                                    if current_active == Some(idx) {
                                        if let Some(handle_id) = overlay_handle_for_hover.get() {
                                            ...
                                            let motion = junita_layout::selector::query_motion(&full_motion_key);
                                ...
                                .on_hover_leave(move |_| {
                                    ...
                                    if let Some(handle_id) = overlay_handle_for_leave.get() {
                                        let mgr = get_overlay_manager();
                                        mgr.close_all_of(junita_layout::widgets::overlay::OverlayKind::Tooltip);
                                        let handle = OverlayHandle::from_raw(handle_id);
...
fn show_navigation_dropdown(
    ...
    key: String,
    surface: junita_core::Color,
    border: junita_core::Color,
    radius: f32,
) -> OverlayHandle {
    use junita_layout::widgets::overlay::AnchorDirection;
...
impl ElementBuilder for NavigationMenu {
    ...
    fn render_props(&self) -> junita_layout::element::RenderProps {
        ...
        let link = stateful_with_key::<ButtonState>(&key)
            .on_state(move |ctx| {
                ...
                let bg = match state {
                    ...
                    _ => junita_core::Color::TRANSPARENT,
                };
