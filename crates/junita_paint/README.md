...
2D drawing primitives and path rendering for Junita UI.

## Overview

`junita_paint` provides a Canvas-like API for 2D drawing, similar to HTML Canvas or Skia.
...
```rust
use junita_paint::{PaintContext, Color, Brush};
...
```rust
use junita_paint::{Path, PathCommand};
...
```rust
use junita_paint::{LinearGradient, RadialGradient, GradientStop};
