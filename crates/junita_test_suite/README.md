...
Visual test suite for Junita UI rendering capabilities.

## Overview

`junita_test_suite` provides a framework for visual regression testing of Junita UI components. It enables automated screenshot comparison to catch unintended visual changes.
...
```rust
use junita_test_suite::{TestRunner, TestConfig, TestContext};
...
cargo test --package junita_test_suite

# Run specific test
cargo test --package junita_test_suite button_

# Update baselines
UPDATE_BASELINES=1 cargo test --package junita_test_suite

# Interactive mode (requires feature)
cargo test --package junita_test_suite --features interactive
```

## Frame Comparison

```rust
use junita_test_suite::compare_frames;
