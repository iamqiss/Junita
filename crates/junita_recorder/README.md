...
Recording and debugging infrastructure for Junita applications.

## Overview

`junita_recorder` provides tools for recording user interactions, capturing UI state, and enabling visual regression testing.
...
```rust
use junita_recorder::{SharedRecordingSession, RecordingConfig};
...
```rust
use junita_recorder::RecordedEvent;
...
```rust
use junita_recorder::ReplayPlayer;
...
```rust
use junita_recorder::{TestRunner, ScreenshotExporter};
...
```rust
use junita_recorder::DebugServer;
