//! junita_recorder - Recording and debugging infrastructure for Junita applications.
//!
//! This crate provides:
...
//!
//! ```ignore
//! use junita_recorder::{install_recorder, RecordingConfig, SharedRecordingSession};
//! use std::sync::Arc;
//!
...
/// Callback types for integration with JunitaContextState.
pub mod callbacks {
    use super::*;
...
/// Install recorder hooks into JunitaContextState.
///
...
///
/// ```ignore
/// use junita_recorder::{install_recorder, install_hooks, RecordingConfig, SharedRecordingSession};
/// use std::sync::Arc;
///
/// let session = Arc::new(SharedRecordingSession::new(RecordingConfig::debug()));
/// install_recorder(session.clone());
...
pub fn install_hooks() {
    ...
    // Only install hooks if JunitaContextState is initialized
    if let Some(ctx) = JunitaContextState::try_get() {
        // Event callback: convert type-erased event to RecordedEvent and record
        let event_callback: junita_core::RecorderEventCallback =
            Arc::new(|event_any: junita_core::RecordedEventAny| {
        ...
        let snapshot_callback: junita_core::RecorderSnapshotCallback =
            Arc::new(|snapshot_any: junita_core::TreeSnapshotAny| {
        ...
        let update_callback: junita_core::RecorderUpdateCallback =
            Arc::new(|element_id: &str, category: junita_core::UpdateCategory| {
                tracing::trace!(
                    target: "junita_recorder::updates",
...
/// Uninstall recorder hooks from JunitaContextState.
...
/// ```ignore
/// fn main() {
///     junita_recorder::enable_debug_recording!();
///     // Your app code...
/// }
/// ```
#[macro_export]
...
/// 3. Installs hooks into JunitaContextState (if available)
...
/// ```ignore
/// fn main() {
///     // Basic usage - uses default app name "junita_app"
///     let (session, server) = junita_recorder::enable_debug_server!();
///
///     // Your app code...
///
...
///     let (session, server) = junita_recorder::enable_debug_server!("my_app");
///
///     // Your app code...
/// }
/// ```
