...
impl DebugServerConfig {
    ...
    pub fn socket_path(&self) -> PathBuf {
        ...
        #[cfg(unix)]
        {
            PathBuf::from(format!("/tmp/junita/{}.sock", self.app_name))
        }

        #[cfg(windows)]
        {
            PathBuf::from(format!(r"\\.\pipe\junita\{}", self.app_name))
        }
    }
}
...
mod tests {
    ...
    fn test_socket_path_unix() {
        ...
        assert_eq!(
            config.socket_path(),
            PathBuf::from("/tmp/junita/test_app.sock")
        );
    }
