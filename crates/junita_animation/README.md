...
Animation system for Junita UI - spring physics, keyframes, and timeline orchestration.

## Overview

`junita_animation` provides a powerful animation system with spring physics, keyframe animations, and timeline orchestration. All animations are interruptible and preserve velocity when interrupted.
...
```rust
use junita_animation::{Spring, SpringConfig};
...
```rust
use junita_animation::{KeyframeAnimation, Keyframe, Easing};
...
```rust
use junita_animation::{Timeline, TimelineEntry};
...
```rust
use junita_animation::AnimatedValue;
