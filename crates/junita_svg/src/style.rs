//! SVG style conversion to Junita types

use junita_core::{Brush, Color, Gradient, GradientStop, LineCap, LineJoin, Stroke};

/// Convert usvg Paint to Junita Brush
    ...
    match paint {
        ...
        usvg::Paint::LinearGradient(lg) => {
            ...
            let gradient = Gradient::linear_with_stops(
                junita_core::Point::new(lg.x1() as f32, lg.y1() as f32),
                junita_core::Point::new(lg.x2() as f32, lg.y2() as f32),
                stops,
            );
        ...
        usvg::Paint::RadialGradient(rg) => {
            ...
            let gradient = Gradient::radial_with_stops(
                junita_core::Point::new(rg.cx() as f32, rg.cy() as f32),
                rg.r().get() as f32,
                stops,
            );
            Some(Brush::Gradient(gradient))
...
/// Convert usvg Fill to Junita Brush
pub fn fill_to_brush(fill: &usvg::Fill) -> Option<Brush> {
    paint_to_brush(fill.paint(), fill.opacity().get())
}

/// Convert usvg Stroke to Junita Stroke and Brush
pub fn stroke_to_junita(stroke: &usvg::Stroke) -> Option<(Stroke, Brush)> {
    ...
    let mut junita_stroke = Stroke::new(stroke.width().get() as f32)
        .with_cap(cap)
        .with_join(join);
    ...
    if let Some(dasharray) = stroke.dasharray() {
        ...
        junita_stroke = junita_stroke.with_dash(dashes, stroke.dashoffset() as f32);
    }

    Some((junita_stroke, brush))
}
