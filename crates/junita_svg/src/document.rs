...
use junita_core::{Brush, DrawContext, Path, PathCommand, Point, Rect, Stroke};
use usvg::{Options, Tree};

use crate::error::SvgError;
use crate::path::usvg_path_to_junita;
use crate::style::{fill_to_brush, stroke_to_junita};
    ...
    fn extract_commands(&self, group: &usvg::Group, commands: &mut Vec<SvgDrawCommand>) {
        for child in group.children() {
            match child {
                ...
                usvg::Node::Path(p) => {
                    // Convert path to Junita path and apply the absolute transform
                    let junita_path = usvg_path_to_junita(p.data());
                    let transformed_path = apply_transform(&junita_path, &p.abs_transform());
                    ...
                    if let Some(stroke) = p.stroke() {
                        if let Some((junita_stroke, brush)) = stroke_to_junita(stroke) {
                            commands.push(SvgDrawCommand::StrokePath {
                                path: transformed_path,
                                stroke: junita_stroke,
                                brush,
                            });
                        }
                    }
...
/// Apply a usvg Transform to a Junita Path
    ...
    let new_commands: Vec<PathCommand> = path
        ...
        .map(|cmd| match cmd {
            ...
            PathCommand::ArcTo {
                ...
                radii: junita_core::Vec2::new(radii.x * scale, radii.y * scale),
                rotation: *rotation,
                large_arc: *large_arc,
                sweep: *sweep,
                end: transform_point(*end),
