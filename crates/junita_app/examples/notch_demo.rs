...
//!
//! Run with: cargo run -p junita_app --example notch_demo --features windowed
...
use junita_layout::stateful::{ButtonState, NoState};
...
fn build_ui(ctx: &WindowedContext) -> impl ElementBuilder {
    ...
    stateful::<NoState>().on_state(move |ctx| {
        ...
        let dropdown_state: junita_core::State<DropdownState> =
...
fn menu_bar(dropdown_state: &junita_core::State<DropdownState>, bg: Color) -> Div {
    let mut bar = div()
        .w_full()
        .h(MENU_BAR_HEIGHT)
        .bg(bg)
...
fn stateful_icon_button(
    item: MenuItem,
    dropdown_state: junita_core::State<DropdownState>,
) -> impl ElementBuilder {
