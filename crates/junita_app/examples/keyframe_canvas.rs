...
//!
//! Run with: cargo run -p junita_app --example keyframe_canvas --features windowed

use junita_animation::context::SharedAnimatedTimeline;
use junita_animation::timeline::TimelineEntryId;
use junita_app::prelude::*;
use junita_app::windowed::{WindowedApp, WindowedContext};
use junita_core::{Brush, Color, CornerRadius, DrawContext, Gradient, Point, Rect};
...
fn pulsing_dots_demo(ctx: &WindowedContext) -> Div {
    ...
    let entry_ids: Vec<(TimelineEntryId, TimelineEntryId)> = timelines
        ...
        .map(|(i, timeline): (usize, &SharedAnimatedTimeline)| {
            timeline
                .lock()
                .unwrap()
                .configure(|t: &mut junita_animation::AnimatedTimeline| {
    ...
    demo_card("Pulsing Dots").child(
        canvas(move |ctx: &mut dyn DrawContext, bounds| {
            ...
            {
                let t: std::sync::MutexGuard<'_, junita_animation::AnimatedTimeline> =
