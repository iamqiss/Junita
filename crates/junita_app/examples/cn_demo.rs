//! junita_cn Components Demo
//!
//! Showcases all available junita_cn components in a scrollable grid layout.
//!
//! Run with: cargo run -p junita_app --example cn_demo --features windowed
...
use junita_layout::selector::ScrollRef;
use junita_layout::widgets::text_input::text_input_data;
use junita_theme::{ColorScheme, ColorToken, ThemeState};
...
fn main() -> Result<()> {
    ...
    let config = WindowConfig {
        title: "junita_cn Components Demo".to_string(),
        width: 900,
        height: 900,
        resizable: true,
        fullscreen: false,
...
fn build_ui(ctx: &WindowedContext) -> impl ElementBuilder {
    ...
    eprintln!(
        ...
        junita_theme::platform::Platform::current()
    );
...
//     use junita_layout::prelude::use_shared_state_with;
...
//     let is_expanded: junita_core::State<bool> =
//         JunitaContextState::get().use_state_keyed("layout_anim_test_expanded", || false);
    ...
    div()
        ...
        .child(
            div()
                ...
                .gap(2.0)
                .child(
                    text("junita_cn Components")
                        .size(theme.typography().text_2xl)
                        .weight(FontWeight::Bold)
                        .color(text_primary),
                )
                .child(
                    text("shadcn-inspired component library for Junita")
...
fn form_inputs_section(_ctx: &WindowedContext) -> impl ElementBuilder {
    ...
    let bio_state = junita_layout::widgets::text_area::text_area_state();
                ...
                .child(
                    div()
                        ...
                        .child(
                            cn::accordion()
                                .default_open("faq-1")
                                .item("faq-1", "What is Junita?", || {
                                    div().w_full().p(4.0).items_center().child(
                                        text("Junita is a Rust UI framework for building beautiful, performant user interfaces with a declarative, GPUI-inspired API.")
                                            .size(14.0)
                                ...
                                .item("faq-2", "How do animations work?", || {
                                    div().w_full().p(4.0).items_center().child(
                                        text("Junita uses spring physics animations via the junita_animation crate. Animations are scheduled through a global scheduler for smooth performance.")
                                            .size(14.0)
                                            .color(ThemeState::get().color(ColorToken::TextSecondary)),
                                    )
                                })
                                .item("faq-3", "Is it production ready?", || {
                                    div().w_full().p(4.0).items_center().child(
                                        text("Junita is under active development. It's suitable for experimentation and side projects, with a growing component library.")
                                            .size(14.0)
                                            .color(ThemeState::get().color(ColorToken::TextSecondary)),
                                    )
                                })
...
fn avatar_section() -> impl ElementBuilder {
    ...
    let avatar_path = "crates/junita_app/examples/assets/avatar.jpg";
