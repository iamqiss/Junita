...
//! - JunitaComponent derive macro for type-safe animation hooks
//!
...
//!
//! Run with: cargo run -p junita_app --example motion_demo --features windowed

use junita_animation::{AnimationPreset, SpringConfig};
use junita_app::prelude::*;
use junita_app::windowed::{WindowedApp, WindowedContext};
use junita_core::Color;
use junita_layout::motion::{motion, StaggerConfig};
use junita_layout::prelude::stateful_from_handle;
use junita_layout::widgets::scroll::Scroll;
use junita_theme::theme;
use std::sync::{Arc, Mutex};

/// Component for the pull-to-refresh demo.
/// The JunitaComponent derive generates type-safe animation hooks.
...
impl junita_layout::prelude::StateTransitions for PullState {
...
fn pull_to_refresh_demo(ctx: &WindowedContext) -> Div {
    // AnimatedValues using JunitaComponent derive macro for type-safe hooks
