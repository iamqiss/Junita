...
//!
//! Run with: cargo run -p junita_app --example canvas_demo --features windowed
...
/// The JunitaComponent derive generates a unique compile-time key and
...
/// With JunitaComponent derive macro for type-safe animation hooks.
fn animated_demo_card(ctx: &WindowedContext) -> Div {
    // AnimatedValue persisted to context using component-based key
    // The JunitaComponent derive macro generates a unique key from the type
