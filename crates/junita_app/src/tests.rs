//! Visual tests for junita_app API
//!
//! Tests render to PNG files in test_output/junita_app/ for visual verification.
//! These tests require a GPU and will be skipped in CI environments without one.

use crate::app::JunitaConfig;
use crate::prelude::*;
use image::{ImageBuffer, Rgba, RgbaImage};
use std::path::Path;

/// Test output directory
const OUTPUT_DIR: &str = "test_output/junita_app";

/// Create test app for rendering tests
/// Returns None if no GPU adapter is available (e.g., in CI without GPU)
fn create_test_app() -> Option<JunitaApp> {
    match JunitaApp::with_config(JunitaConfig {
...
fn test_text_element() {
    ...
    let ui = div()
        ...
        .justify_center()
        .bg(Color::WHITE)
        .child(text("Hello Junita!").size(32.0).color(Color::BLACK));
