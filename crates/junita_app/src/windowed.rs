...
//!
//! Provides a unified API for running windowed Junita applications across
...
use crate::app::JunitaApp;
use crate::error::{JunitaError, Result};

/// Shared animation scheduler for the application (thread-safe)
pub type SharedAnimationScheduler = Arc<Mutex<AnimationScheduler>>;

// SharedAnimatedValue and SharedAnimatedTimeline are re-exported from junita_animation
...
pub type SharedElementRegistry = Arc<junita_layout::selector::ElementRegistry>;
...
impl WindowedContext {
    ...
    where
        ...
        F: FnOnce() -> T,
    {
        use junita_core::reactive::SignalId;
        ...
        let callback: StatefulDepsCallback = Arc::new(|signal_ids| {
            junita_layout::check_stateful_deps(signal_ids);
        });
    ...
    /// }
    /// ```
    pub fn use_scroll_ref(&self, key: &str) -> junita_layout::selector::ScrollRef {
        use junita_core::reactive::SignalId;
        use junita_layout::selector::{ScrollRef, SharedScrollRefInner, TriggerCallback};
    ...
    /// });
    /// ```
    pub fn use_effect<F>(&self, run: F) -> junita_core::reactive::Effect
    where
    ...
    /// }
    /// ```
    pub fn query(&self, id: &str) -> junita_layout::selector::ElementHandle<()> {
        junita_layout::selector::ElementHandle::new(id, self.element_registry.clone())
    }
    ...
    /// ```
    #[track_caller]
    pub fn use_state<S>(&self, initial: S) -> junita_layout::SharedState<S>
    where
        S: junita_layout::StateTransitions + Clone + Send + 'static,
    {
    ...
    /// }
    /// ```
    pub fn use_state_for<K, S>(&self, key: K, initial: S) -> junita_layout::SharedState<S>
    where
        K: Hash,
        S: junita_layout::StateTransitions + Clone + Send + 'static,
    {
        use junita_core::reactive::SignalId;
        use junita_layout::stateful::StatefulInner;

        // We store the SharedState<S> as a signal value
        let state_key = StateKey::new::<junita_layout::SharedState<S>, _>(&key);
        let mut hooks = self.hooks.lock().unwrap();

        if let Some(raw_id) = hooks.get(&state_key) {
            // Existing state - get the SharedState from the signal
            let signal_id = SignalId::from_raw(raw_id);
            let signal: Signal<junita_layout::SharedState<S>> = Signal::from_id(signal_id);
            self.reactive.lock().unwrap().get(signal).unwrap()
        } else {
            // New state - create SharedState and store in signal
            let shared_state: junita_layout::SharedState<S> =
    ...
    /// ```
    #[track_caller]
    pub fn use_tick_callback<F>(&self, callback: F) -> junita_animation::TickCallbackId
    where
    ...
    pub fn use_tick_callback_for<K: Hash, F>(
        ...
        key: K,
        callback: F,
    ) -> junita_animation::TickCallbackId
    where
        F: FnMut(f32) + Send + Sync + 'static,
    {
        ...
        if let Some(raw_id) = hooks.get(&state_key) {
            ...
            junita_animation::TickCallbackId::from_raw(raw_id)
        } else {
    ...
    /// }
    /// ```
    pub fn color_scheme(&self) -> junita_theme::ColorScheme {
        junita_theme::ThemeState::get().scheme()
    }
    ...
    /// ```
    pub fn set_color_scheme(&self, scheme: junita_theme::ColorScheme) {
        junita_theme::ThemeState::get().set_scheme(scheme);
    }
    ...
    pub fn toggle_color_scheme(&self) {
        junita_theme::ThemeState::get().toggle_scheme();
    }
    ...
    ///
    /// ```ignore
    /// use junita_theme::ColorToken;
    ///
    /// let primary = ctx.theme_color(ColorToken::Primary);
    /// let bg = ctx.theme_color(ColorToken::Background);
    /// ```
    pub fn theme_color(&self, token: junita_theme::ColorToken) -> junita_core::Color {
        junita_theme::ThemeState::get().color(token)
    }
    ...
    ///
    /// ```ignore
    /// use junita_theme::SpacingToken;
    ///
    /// let padding = ctx.theme_spacing(SpacingToken::Space4); // 16px
    /// ```
    pub fn theme_spacing(&self, token: junita_theme::SpacingToken) -> f32 {
        junita_theme::ThemeState::get().spacing_value(token)
    }
    ...
    ///
    /// ```ignore
    /// use junita_theme::RadiusToken;
    ///
    /// let radius = ctx.theme_radius(RadiusToken::Lg); // 8px
    /// ```
    pub fn theme_radius(&self, token: junita_theme::RadiusToken) -> f32 {
        junita_theme::ThemeState::get().radius(token)
    }
}

// =============================================================================
// JunitaContext Implementation
// =============================================================================

impl junita_core::JunitaContext for WindowedContext {
    ...
    fn dirty_flag(&self) -> junita_core::DirtyFlag {
...
/// Windowed application runner
///
/// Provides a simple way to run a Junita application in a window
...
impl WindowedApp {
    ...
    fn init_asset_loader() {
        use junita_platform::assets::{set_global_asset_loader, FilesystemAssetLoader};
    ...
    fn init_theme() {
        ...
        set_redraw_callback(|| {
            ...
            junita_layout::widgets::request_full_rebuild();
        });
    }

    /// Run a windowed Junita application on desktop platforms
    ///
        ...
        E: ElementBuilder + 'static,
    {
        ...
        let platform = DesktopPlatform::new().map_err(|e| JunitaError::Platform(e.to_string()))?;
        let event_loop = platform
            .create_event_loop_with_config(config)
            .map_err(|e| JunitaError::Platform(e.to_string()))?;

        // Get a wake proxy to allow the animation thread to wake up the event loop
        let wake_proxy = event_loop.wake_proxy();

        // We need to defer JunitaApp creation until we have a window
        let mut app: Option<JunitaApp> = None;
        ...
        if !JunitaContextState::is_initialized() {
            let stateful_callback: std::sync::Arc<dyn Fn(&[SignalId]) + Send + Sync> =
                Arc::new(|signal_ids| {
                    junita_layout::check_stateful_deps(signal_ids);
                });
            JunitaContextState::init_with_callback(
                Arc::clone(&reactive),
                Arc::clone(&hooks),
        ...
        // Set global scheduler handle for StateContext and component access
        {
            ...
            junita_animation::set_global_scheduler(scheduler_handle);
        }

        // Shared element registry for query API
        let element_registry: SharedElementRegistry =
            Arc::new(junita_layout::selector::ElementRegistry::new());

        // Set up query callback in JunitaContextState so components can query elements globally
        {
            let registry_for_query = Arc::clone(&element_registry);
            let query_callback: junita_core::QueryCallback = Arc::new(move |id: &str| {
                registry_for_query.get(id).map(|node_id| node_id.to_raw())
            });
            JunitaContextState::get().set_query_callback(query_callback);
        }

        // Set up bounds callback for ElementHandle.bounds()
        {
            let registry_for_bounds = Arc::clone(&element_registry);
            let bounds_callback: junita_core::BoundsCallback =
                Arc::new(move |id: &str| registry_for_bounds.get_bounds(id));
            JunitaContextState::get().set_bounds_callback(bounds_callback);
        }

        // Store element registry in JunitaContextState for global query() function
        // Cast to Arc<dyn Any + Send + Sync> for type-erased storage
        JunitaContextState::get()
            .set_element_registry(Arc::clone(&element_registry) as junita_core::AnyElementRegistry);
        ...
        // This bridges text widgets (which track focus) with the animation scheduler (which drives redraws)
        {
            ...
            junita_layout::widgets::set_continuous_redraw_callback(move |enabled| {
        ...
        junita_theme::ThemeState::get().set_scheduler(&animations);
        ...
        let mut render_state: Option<junita_layout::RenderState> = None;

        // Shared motion states for query API access
        // This allows components to query motion animation state via query_motion()
        let shared_motion_states = junita_layout::create_shared_motion_states();

        // Set up motion state callback in JunitaContextState
        {
            let motion_states_for_callback = Arc::clone(&shared_motion_states);
            let motion_callback: junita_core::MotionStateCallback = Arc::new(move |key: &str| {
                motion_states_for_callback
                    .read()
                    .ok()
                    .and_then(|states| states.get(key).copied())
                    .unwrap_or(junita_core::MotionAnimationState::NotFound)
            });
            JunitaContextState::get().set_motion_state_callback(motion_callback);
        }
                ...
                match event {
                    Event::Lifecycle(LifecycleEvent::Resumed) => {
                        ...
                        if app.is_none() {
                            ...
                            match JunitaApp::with_window(winit_window, None) {
                                Ok((junita_app, surf)) => {
                                    let (width, height) = window.size();
                                    // Use the same texture format that the renderer's pipelines use
                                    let format = junita_app.texture_format();
                                    ...
                                    surf.configure(&junita_app.device(), &config);
                                    ...
                                    // Set initial viewport size in JunitaContextState
                                    if let Some(ref windowed_ctx) = ctx {
                                        JunitaContextState::get().set_viewport_size(windowed_ctx.width, windowed_ctx.height);
                                    }
                                    ...
                                    let mut rs = junita_layout::RenderState::new(Arc::clone(&animations));
                                    rs.set_shared_motion_states(Arc::clone(&shared_motion_states));
                                    render_state = Some(rs);

                                    tracing::debug!("Junita windowed app initialized");
                                }
                                Err(e) => {
                                    tracing::error!("Failed to initialize Junita: {}", e);
                    ...
                    Event::Window(WindowEvent::Resized { width, height }) => {
                        if let (Some(ref junita_app), Some(ref surf), Some(ref mut config)) =
                            (&app, &surface, &mut surface_config)
                        {
                            if width > 0 && height > 0 {
                                    ...
                                    (&mut ctx, &render_tree)
                                {
                                    ...
                                    // Update viewport size in JunitaContextState for ElementHandle.is_visible()
                                    JunitaContextState::get().set_viewport_size(logical_width, logical_height);
                    ...
                    Event::Window(WindowEvent::Focused(focused)) => {
                        ...
                        if let Some(ref mut windowed_ctx) = ctx {
                            ...
                            if !focused {
                                junita_layout::widgets::blur_all_text_inputs();
                            }
                        }
                    }
                            ...
                            (&mut ctx, &render_tree)
                        {
                            ...
                            match input_event {
                                InputEvent::Mouse(mouse_event) => match mouse_event {
                                    MouseEvent::Moved { x, y } => {
                                        ...
                                        let overlay_layer_id = tree.query_by_id(
                                            junita_layout::widgets::overlay::OVERLAY_LAYER_ID
                                        );
                                        ...
                                        for event in pending_events.iter_mut() {
                                            ...
                                            if event.event_type == junita_core::events::event_types::DRAG
                                                || event.event_type == junita_core::events::event_types::DRAG_END
                                            {
                                    ...
                                    MouseEvent::Left => {
                                        ...
                                        window.set_cursor(junita_platform::Cursor::Default);
                                ...
                                InputEvent::Keyboard(kb_event) => {
                                    ...
                                    match kb_event.state {
                                        KeyState::Pressed => {
                                            ...
                                            if let Some(c) = key_char {
                                                ...
                                                if !mods.ctrl && !mods.meta {
                                                    keyboard_events.push(PendingEvent {
                                                        event_type: junita_core::events::event_types::TEXT_INPUT,
                                                        key_char: Some(c),
                                                        key_code,
                                                        shift: mods.shift,
                                                        ctrl: mods.ctrl,
                                            ...
                                            if key_code != 0 {
                                                keyboard_events.push(PendingEvent {
                                                    event_type: junita_core::events::event_types::KEY_DOWN,
                                                    key_char: None,
                                                    key_code,
                                                    shift: mods.shift,
                                                    ctrl: mods.ctrl,
                                ...
                                InputEvent::Scroll { delta_x, delta_y, phase } => {
                                    ...
                                    if phase == junita_platform::ScrollPhase::Ended {
                                        gesture_ended = true;
                                    }
                        ...
                        if let Some(ref mut tree) = render_tree {
                            ...
                            if let Some(ref mut windowed_ctx) = ctx {
                                ...
                                for event in pending_events {
                                    ...
                                    if event.event_type == junita_core::events::event_types::SCROLL {
                                        continue;
                                    }
                            ...
                            for event in keyboard_events {
                                if event.event_type == junita_core::events::event_types::TEXT_INPUT {
                        ...
                        ) = (&mut app, &surface, &surface_config, &mut ctx, &mut render_state)
                        {
                            ...
                            if overlay_content_dirty {
                                ...
                                if let Some(overlay_node_id) = element_registry.get(
                                    ...
                                    junita_layout::queue_subtree_rebuild(overlay_node_id, overlay_content);
                                } else {
                            ...
                            let has_stateful_updates = junita_layout::take_needs_redraw();
                            let has_pending_rebuilds = junita_layout::has_pending_subtree_rebuilds();
                                ...
                                if let Some(ref mut existing_tree) = render_tree {
                                    if needs_relayout {
                                        ...
                                        *existing_tree = tree;
                                        ...
                                        use junita_layout::UpdateResult;
                            ...
                            let theme_animating = junita_theme::ThemeState::get().tick();
                            ...
                            if let Some(ref tree) = render_tree {
                                ...
                                let result = junita_app.render_tree_with_motion(
                                    tree,
                                    rs,
                                    &view,
                            ...
                            if needs_animation_redraw && junita_layout::has_animating_statefuls() {
                                junita_layout::check_stateful_animations();
                            }

                            // Check if text widgets need continuous redraws (cursor blink)
                            let needs_cursor_redraw = junita_layout::widgets::take_needs_continuous_redraw();
                            ...
                            if needs_animation_redraw || needs_cursor_redraw || needs_motion_redraw || scroll_animating || needs_overlay_redraw || theme_animating {
                                ...
                                if needs_cursor_redraw {
                                    ...
                                    if junita_layout::widgets::has_focused_text_input() {
                                        junita_layout::widgets::text_input::request_continuous_redraw_pub();
                                    }
                                }
                                window.request_redraw();
                            }
    ...
    where
        ...
        E: ElementBuilder + 'static,
    {
        Err(JunitaError::Platform(
            "Windowed feature not enabled. Add 'windowed' feature to junita_app".to_string(),
        ))
    }
}
...
#[cfg(all(feature = "windowed", not(target_os = "android")))]
fn convert_mouse_button(button: junita_platform::MouseButton) -> MouseButton {
    match button {
        junita_platform::MouseButton::Left => MouseButton::Left,
        junita_platform::MouseButton::Right => MouseButton::Right,
        junita_platform::MouseButton::Middle => MouseButton::Middle,
        junita_platform::MouseButton::Back => MouseButton::Back,
        junita_platform::MouseButton::Forward => MouseButton::Forward,
        junita_platform::MouseButton::Other(n) => MouseButton::Other(n),
    }
}

/// Convert layout cursor style to platform cursor
#[cfg(all(feature = "windowed", not(target_os = "android")))]
fn convert_cursor_style(cursor: CursorStyle) -> junita_platform::Cursor {
    match cursor {
        CursorStyle::Default => junita_platform::Cursor::Default,
        CursorStyle::Pointer => junita_platform::Cursor::Pointer,
        CursorStyle::Text => junita_platform::Cursor::Text,
        CursorStyle::Crosshair => junita_platform::Cursor::Crosshair,
        CursorStyle::Move => junita_platform::Cursor::Move,
        CursorStyle::NotAllowed => junita_platform::Cursor::NotAllowed,
        CursorStyle::ResizeNS => junita_platform::Cursor::ResizeNS,
        CursorStyle::ResizeEW => junita_platform::Cursor::ResizeEW,
        CursorStyle::ResizeNESW => junita_platform::Cursor::ResizeNESW,
        CursorStyle::ResizeNWSE => junita_platform::Cursor::ResizeNWSE,
        CursorStyle::Grab => junita_platform::Cursor::Grab,
        CursorStyle::Grabbing => junita_platform::Cursor::Grabbing,
        CursorStyle::Wait => junita_platform::Cursor::Wait,
        CursorStyle::Progress => junita_platform::Cursor::Progress,
        CursorStyle::None => junita_platform::Cursor::None,
    }
}
