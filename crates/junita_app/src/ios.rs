...
//!
//! Provides a unified API for running Junita applications on iOS.
//!
//! # Example
//!
//! ```ignore
//! use junita_app::prelude::*;
//! use junita_app::ios::IOSApp;
//!
...
//! # iOS Integration
//!
//! Unlike Android where Junita can run as a native activity, on iOS you must
//! integrate Junita into your existing UIKit application. The typical flow is:
//!
...
use junita_layout::widgets::overlay::{overlay_manager, OverlayManager};
use junita_platform::assets::set_global_asset_loader;
use junita_platform_ios::{Gesture, GestureDetector, IOSAssetLoader, IOSWakeProxy, TouchPhase};
...
/// iOS application runner
///
/// Provides methods for running a Junita application on iOS with Metal rendering.
/// Unlike desktop or Android, iOS apps must integrate Junita into their existing
...
impl IOSApp {
    ...
    /// Create a new Junita context for iOS rendering
    ///
    /// This sets up all the shared state needed for Junita rendering.
    ...
    pub fn create_context(width: u32, height: u32, scale_factor: f64) -> Result<IOSRenderContext> {
        ...
        // Store element registry in JunitaContextState
        ...
        let mut render_state = junita_layout::RenderState::new(Arc::clone(&animations));
    ...
    pub fn system_font_paths() -> &'static [&'static str] {
        junita_platform_ios::system_font_paths()
    }
}

/// iOS render context
///
/// Holds all the state needed to render Junita UI on iOS.
...
pub struct IOSRenderContext {
    ...
    render_state: junita_layout::RenderState,
...
impl IOSRenderContext {
    ...
    pub fn needs_render(&self) -> bool {
        ...
        let has_stateful_updates = junita_layout::peek_needs_redraw();
    ...
    pub fn tick_scroll(&mut self) -> bool {
        if let Some(ref mut tree) = self.render_tree {
            let current_time = junita_layout::prelude::elapsed_ms();
    ...
    pub fn render_state(&self) -> &junita_layout::RenderState {
        &self.render_state
    }
    ...
    ///         junita_handle_touch(context, 0, Float(point.x), Float(point.y), 0) // 0 = began
    ///     }
    /// }
    /// ```
    pub fn handle_touch(&mut self, touch: junita_platform_ios::Touch) {
        use junita_layout::tree::LayoutNodeId;
        ...
        let tree = match &self.render_tree {
            Some(t) => t,
            None => {
                tracing::debug!("[Junita] iOS handle_touch: No render tree yet, ignoring touch");
                return;
            }
        };
        ...
        if let Some(root) = tree.root() {
            if let Some(bounds) = tree.layout().get_bounds(root, (0.0, 0.0)) {
                tracing::trace!(
                    "[Junita] iOS Touch at ({:.1}, {:.1}) - tree root bounds: ({:.1}, {:.1}, {:.1}x{:.1})",
                    lx, ly, bounds.x, bounds.y, bounds.width, bounds.height
                );
            } else {
                tracing::debug!("[Junita] iOS Touch: tree root has no bounds!");
            }
        } else {
            tracing::debug!("[Junita] iOS Touch: tree has no root!");
        }
        ...
        match touch.phase {
            TouchPhase::Began => {
                tracing::trace!("[Junita] iOS Touch BEGAN at ({:.1}, {:.1})", lx, ly);
                self.windowed_ctx
                    .event_router
            ...
            TouchPhase::Ended => {
                tracing::trace!("[Junita] iOS Touch ENDED at ({:.1}, {:.1})", lx, ly);
                self.windowed_ctx
                    .event_router
            ...
            TouchPhase::Cancelled => {
                tracing::trace!("[Junita] iOS Touch CANCELLED");
                self.windowed_ctx.event_router.on_mouse_leave();
        ...
        tracing::trace!(
            "[Junita] iOS Touch: collected {} pending events",
            pending_events.len()
        );

        // Dispatch collected events to the tree
        if !pending_events.is_empty() {
            tracing::trace!("[Junita] iOS dispatching {} events", pending_events.len());
...
///     junita_app::ios::register_rust_ui_builder(|ctx| {
///         my_app_ui(ctx)
///     });
/// }
/// ```
...
#[no_mangle]
pub extern "C" fn junita_set_ui_builder(builder: UIBuilderFn) {
    unsafe {
...
/// full rebuilds. Call this each frame when junita_needs_render() is true.
///
/// The function:
...
///
/// # Safety
/// `ctx` must be a valid pointer returned by `junita_create_context`.
/// A UI builder must have been registered via `junita_set_ui_builder` or `register_rust_ui_builder`.
#[no_mangle]
pub extern "C" fn junita_build_frame(ctx: *mut IOSRenderContext) {
    if ctx.is_null() {
        return;
    }
...
///
/// # Safety
/// The returned pointer must be freed with `junita_destroy_context`.
#[no_mangle]
pub extern "C" fn junita_create_context(
    width: u32,
    height: u32,
    scale_factor: f64,
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_needs_render(ctx: *mut IOSRenderContext) -> bool {
    if ctx.is_null() {
        return false;
    }
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_update_size(
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_handle_touch(
    ctx: *mut IOSRenderContext,
    touch_id: u64,
    x: f32,
    y: f32,
    ...
    tracing::trace!(
        "[Junita FFI] junita_handle_touch called: x={}, y={}, phase={}",
        x,
        y,
        phase
    );
    ...
    if ctx.is_null() {
        tracing::debug!("[Junita FFI] junita_handle_touch: ctx is NULL!");
        return;
    }
    ...
    let touch = junita_platform_ios::Touch::new(touch_id, x, y, touch_phase);
    unsafe {
        (*ctx).handle_touch(touch);
    }
    tracing::trace!("[Junita FFI] junita_handle_touch completed");
}
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_set_focused(ctx: *mut IOSRenderContext, focused: bool) {
    if ctx.is_null() {
        return;
    }
    unsafe {
...
#[no_mangle]
pub extern "C" fn junita_destroy_context(ctx: *mut IOSRenderContext) {
    if !ctx.is_null() {
        unsafe {
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_tick_animations(ctx: *mut IOSRenderContext) -> bool {
    if ctx.is_null() {
        return false;
    }
    unsafe {
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_get_width(ctx: *mut IOSRenderContext) -> f32 {
    if ctx.is_null() {
        return 0.0;
    }
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_get_height(ctx: *mut IOSRenderContext) -> f32 {
    if ctx.is_null() {
        return 0.0;
    }
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_get_scale_factor(ctx: *mut IOSRenderContext) -> f64 {
    if ctx.is_null() {
        return 1.0;
    }
...
#[no_mangle]
pub extern "C" fn junita_get_windowed_context(ctx: *mut IOSRenderContext) -> *mut WindowedContext {
    if ctx.is_null() {
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_mark_dirty(ctx: *mut IOSRenderContext) {
    if ctx.is_null() {
        return;
    }
    unsafe {
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_clear_dirty(ctx: *mut IOSRenderContext) {
    if ctx.is_null() {
        return;
    }
    unsafe {
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_get_physical_width(ctx: *mut IOSRenderContext) -> u32 {
    if ctx.is_null() {
        return 0;
    }
    unsafe {
...
/// `ctx` must be a valid pointer returned by `junita_create_context`.
#[no_mangle]
pub extern "C" fn junita_get_physical_height(ctx: *mut IOSRenderContext) -> u32 {
    if ctx.is_null() {
        return 0;
    }
    unsafe {
...
pub struct IOSGpuRenderer {
    /// The Junita application (includes renderer, text context, image context)
    app: JunitaApp,
    /// The wgpu surface
...
///
/// # Arguments
/// * `ctx` - Render context pointer from `junita_create_context`
...
#[no_mangle]
pub extern "C" fn junita_init_gpu(
    ctx: *mut IOSRenderContext,
    metal_layer: *mut std::ffi::c_void,
    width: u32,
    height: u32,
) -> *mut IOSGpuRenderer {
    use junita_gpu::{GpuRenderer, RendererConfig, TextRenderingContext};

    if ctx.is_null() || metal_layer.is_null() {
        tracing::error!("junita_init_gpu: null context or metal_layer");
        return std::ptr::null_mut();
    }

    let config = crate::JunitaConfig::default();
    ...
    let surface = match unsafe { instance.create_surface_unsafe(surface_target) } {
        Ok(s) => s,
        Err(e) => {
            tracing::error!("junita_init_gpu: failed to create surface: {}", e);
            return std::ptr::null_mut();
        }
    };
    ...
    let renderer = match pollster::block_on(async {
        ...
        Ok(r) => r,
        Err(e) => {
            tracing::error!("junita_init_gpu: failed to create renderer: {}", e);
            return std::ptr::null_mut();
        }
    };
    ...
    text_ctx.preload_generic_styles(junita_gpu::GenericFont::SansSerif, &[400, 700], false);
    ...
    let app = JunitaApp::from_context(render_context, config);
    ...
    tracing::info!(
        "junita_init_gpu: GPU initialized ({}x{}, format: {:?})",
        width,
        height,
        format
    );
...
///
/// # Safety
/// `gpu` must be a valid pointer returned by `junita_init_gpu`.
#[no_mangle]
pub extern "C" fn junita_gpu_resize(gpu: *mut IOSGpuRenderer, width: u32, height: u32) {
    if gpu.is_null() {
        return;
    }
    ...
    unsafe {
            ...
            && (gpu.surface_config.width != width || gpu.surface_config.height != height)
        {
            ...
            tracing::debug!("junita_gpu_resize: {}x{}", width, height);
        }
    }
}
...
/// Call this from your CADisplayLink callback when `junita_needs_render()` is true.
///
/// # Returns
...
#[no_mangle]
pub extern "C" fn junita_render_frame(gpu: *mut IOSGpuRenderer) -> bool {
    if gpu.is_null() {
        return false;
    }
    ...
    unsafe {
        ...
        let surface_texture = match gpu.surface.get_current_texture() {
            ...
            Err(wgpu::SurfaceError::Lost | wgpu::SurfaceError::Outdated) => {
                ...
                match gpu.surface.get_current_texture() {
                    Ok(st) => st,
                    Err(e) => {
                        tracing::error!("junita_render_frame: surface error: {:?}", e);
                        return false;
                    }
                }
            }
        ...
        if let Err(e) = gpu.app.render_tree_with_motion(
            ...
            tracing::error!("junita_render_frame: render error: {}", e);
            surface_texture.present();
            return false;
        }
...
/// and must not be used after this call.
#[no_mangle]
pub extern "C" fn junita_destroy_gpu(gpu: *mut IOSGpuRenderer) {
    if !gpu.is_null() {
        unsafe {
...
///
/// Call this after `junita_init_gpu` to load fonts from the app bundle.
/// The font will be added to the font registry and available for text rendering.
///
/// # Arguments
/// * `gpu` - GPU renderer pointer from `junita_init_gpu`
...
#[no_mangle]
pub extern "C" fn junita_load_bundled_font(
    ...
    unsafe {
        ...
        let path_str = match std::ffi::CStr::from_ptr(path).to_str() {
            Ok(s) => s,
            Err(_) => {
                tracing::error!("junita_load_bundled_font: invalid path string");
                return 0;
            }
        };
        ...
        if !path.exists() {
            tracing::error!(
                "junita_load_bundled_font: font file does not exist: {}",
                path_str
            );
            return 0;
        }
        ...
        match std::fs::read(path) {
            ...
            Err(e) => {
                tracing::error!("junita_load_bundled_font: failed to read font: {}", e);
                0
            }
        }
    }
