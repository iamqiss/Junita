...
//!
//! Provides a unified API for running Junita applications on Android.
//!
//! # Example
//!
//! ```ignore
//! use junita_app::prelude::*;
//! use junita_app::android::AndroidApp;
//!
//! #[no_mangle]
...
use junita_platform_android::input::{detect_pinch, PinchPhase, PinchState, TouchPointer};
use junita_platform_android::AndroidAssetLoader;
...
/// Android application runner
///
/// Provides a simple way to run a Junita application on Android
...
impl AndroidApp {
    ...
    fn init_logging() {
        ...
        let subscriber =
            tracing_subscriber::registry().with(tracing_android::layer("Junita").unwrap());
        let _ = tracing::subscriber::set_global_default(subscriber);
    }

    /// Run an Android Junita application
    ///
        ...
        E: ElementBuilder + 'static,
    {
        ...
        // Application state
        let mut junita_app: Option<JunitaApp> = None;
        ...
        while running {
                            ...
                            if let Some(window) = app.native_window() {
                                ...
                                match Self::init_gpu(&window) {
                                    Ok((app_instance, surf)) => {
                                        ...
                                        junita_app = Some(app_instance);
                                        native_window = Some(window);

                                        // Create WindowedContext with actual display density
                                        let scale_factor =
                                            junita_platform_android::get_display_density(&app);
                        ...
                        MainEvent::WindowResized { .. } => {
                            if let Some(ref window) = native_window {
                                ...
                                if let (
                                    ...
                                    Some(ref surf),
                                    Some(ref mut config),
                                ) = (&junita_app, &surface, &mut surface_config)
                                {
                                    if width > 0 && height > 0 {
                                        config.width = width;
                                        ...
                                        if let Some(ref mut windowed_ctx) = ctx {
                                            ...
                                            JunitaContextState::get().set_viewport_size(
                                                windowed_ctx.width,
                                                windowed_ctx.height,
                                            );
                                        }
            ...
            struct PendingEvent {
                node_id: junita_layout::LayoutNodeId,
                event_type: u32,
                mouse_x: f32,
                mouse_y: f32,
            }
            ...
            if let Some((mouse_x, mouse_y, delta_x, delta_y)) = scroll_info {
                    ...
                    (&mut ctx, &mut render_tree)
                {
                    ...
                    if let Some(hit) = router.hit_test(tree, mouse_x, mouse_y) {
                        ...
                        let scroll_time = junita_layout::prelude::elapsed_ms() as f64;
                        tracing::debug!(
            ...
            // =========================================================
            {
                ...
                if junita_layout::has_animating_statefuls() {
                    needs_redraw_next_frame = true;
                }

                // Check for pending subtree rebuilds that might need processing
                if junita_layout::has_pending_subtree_rebuilds() {
                    needs_redraw_next_frame = true;
                }
            }
        }
    ...
    fn init_gpu(window: &NativeWindow) -> Result<(JunitaApp, wgpu::Surface<'static>)> {
        ...
        let raw_window = NonNull::new(window.ptr().as_ptr() as *mut std::ffi::c_void)
            .ok_or_else(|| JunitaError::GpuInit("Invalid native window pointer".to_string()))?;
        ...
        let surface = unsafe {
            instance
                ...
                .map_err(|e| JunitaError::GpuInit(e.to_string()))?
        };
        ...
        let app = JunitaApp::from_context(ctx, config);
