...
use junita_layout::text_measure::{TextLayoutOptions, TextMeasurer, TextMetrics};
use junita_layout::GenericFont as LayoutGenericFont;
use junita_text::{FontFace, FontRegistry, GenericFont, LayoutOptions, TextLayoutEngine};
...
impl FontTextMeasurer {
    ...
    pub fn new() -> Self {
        let mut measurer = Self {
            ...
            font_registry: junita_text::global_font_registry(),
    ...
    pub fn load_font_data(&self, data: Vec<u8>) -> Result<(), junita_text::TextError> {
...
impl TextMeasurer for FontTextMeasurer {
    fn measure_with_options(
        ...
        // Convert our options to junita_text options
        ...
        if let Some(max_width) = options.max_width {
            ...
            layout_opts.line_break = junita_text::LineBreakMode::None;
        }
...
pub fn init_text_measurer() {
    ...
    junita_layout::set_text_measurer(measurer);
}
...
///
/// Call this after creating the JunitaApp/TextRenderingContext:
///
/// ```ignore
/// let (app, surface) = JunitaApp::with_window(window, None)?;
