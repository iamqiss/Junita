...
use junita_layout::div::{FontFamily, FontWeight, GenericFont, TextAlign, TextVerticalAlign};
use junita_layout::prelude::*;
use junita_layout::render_state::Overlay;
use junita_layout::renderer::ElementType;
use junita_svg::{RasterizedSvg, SvgDocument};
use lru::LruCache;
...
struct ImageElement {
    ...
    border_width: f32,
    /// Border color
    border_color: junita_core::Color,
}
...
impl RenderContext {
    ...
    ///
    /// When `JUNITA_DEBUG=text` (or `1`, `all`, `true`) is set, this renders:
    ...
    ///
    /// When `JUNITA_DEBUG=layout` (or `all`) is set, this renders:
    ...
    ///
    /// When `JUNITA_DEBUG=motion` (or `all`) is set, this renders:
    ...
    fn preload_images(
        ...
        for image in images {
            ...
            let source = junita_image::ImageSource::from_uri(&image.source);
            let image_data = match junita_image::ImageData::load(source) {
                Ok(data) => data,
                Err(e) => {
    ...
    fn render_images(
        ...
        use junita_image::{calculate_fit_rects, src_rect_to_uv, ObjectFit, ObjectPosition};
        ...
        for image in images {
            ...
            if gpu_image.is_none() && image.placeholder_type != 0 {
                ...
                if image.placeholder_type == 1 {
                    ...
                    let color = junita_core::Color::rgba(
                    ...
                    let rect = junita_core::Rect::new(image.x, image.y, image.width, image.height);

                    ctx.fill_rounded_rect(
                        rect,
                        junita_core::CornerRadius::uniform(image.border_radius),
                        color,
                    );
            ...
            if image.border_width > 0.0 {
                use junita_gpu::primitives::GpuPrimitive;
    ...
    fn render_svg_element(&mut self, ctx: &mut GpuPaintContext, svg: &SvgElement) {
        ...
        if let Some([clip_x, clip_y, clip_w, clip_h]) = svg.clip_bounds {
            ctx.push_clip(junita_core::ClipShape::rect(Rect::new(
    ...
    fn render_svg_with_tint(
        ...
        width: f32,
        height: f32,
        tint: junita_core::Color,
    ) {
        use junita_svg::SvgDrawCommand;
    ...
    fn collect_render_elements_with_state(
        &mut self,
        tree: &RenderTree,
        render_state: Option<&junita_layout::RenderState>,
    ) -> (Vec<TextElement>, Vec<SvgElement>, Vec<ImageElement>) {
    ...
    fn collect_elements_recursive(
            ...
            match &render_node.element_type {
                ...
                ElementType::Image(image_data) => {
                    ...
                    images.push(ImageElement {
                        ...
                        border_color: render_node
                            .props
                            .border_color
                            .unwrap_or(junita_core::Color::TRANSPARENT),
                    });
                }
                ...
                ElementType::StyledText(styled_data) => {
                    ...
                    for (start, end, color, bold, italic, underline, strikethrough) in segments {
                        ...
                        let mut options = junita_layout::text_measure::TextLayoutOptions::new();
                        ...
                        let metrics = junita_layout::text_measure::measure_text_with_options(
                            segment_text,
        ...
        let static_motion_offset = tree
            ...
            .map(|t| match t {
                junita_core::Transform::Affine2D(a) => (a.elements[4], a.elements[5]),
                _ => (0.0, 0.0),
            })
            .unwrap_or((0.0, 0.0));
    ...
    pub fn render_tree_with_motion(
        ...
        // Render debug visualization if enabled (JUNITA_DEBUG=text|layout|all)
...
///
/// Set environment variable `JUNITA_DEBUG` to enable debug visualization:
...
impl DebugMode {
    ...
    pub fn from_env() -> Self {
        let debug_value = std::env::var("JUNITA_DEBUG")
...
fn scale_and_translate_path(
    path: &junita_core::Path,
    x: f32,
    y: f32,
    scale: f32,
) -> junita_core::Path {
    use junita_core::{PathCommand, Point, Vec2};
    ...
    junita_core::Path::from_commands(new_commands)
}
