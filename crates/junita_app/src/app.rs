//! Junita Application Delegate
//!
//! The main entry point for Junita applications.

use junita_gpu::{FontRegistry, GpuRenderer, RendererConfig, TextRenderingContext};
use junita_layout::prelude::*;
use junita_layout::RenderTree;
use std::sync::{Arc, Mutex};

use crate::context::RenderContext;
use crate::error::{JunitaError, Result};

/// Junita application configuration
#[derive(Clone, Debug)]
...
/// The main Junita application
///
/// This is the primary interface for rendering Junita UI.
...
/// ```
pub struct JunitaApp {
    ctx: RenderContext,
    config: JunitaConfig,
}

impl JunitaApp {
    /// Create a new Junita application with default configuration
    pub fn new() -> Result<Self> {
        Self::with_config(JunitaConfig::default())
    }

    /// Create a new Junita application from an existing render context
    ///
    /// This is used internally for platform-specific initialization (Android, iOS)
    /// where the GPU setup is done differently.
    pub(crate) fn from_context(ctx: RenderContext, config: JunitaConfig) -> Self {
        Self { ctx, config }
    }

    /// Create a new Junita application with custom configuration
    pub fn with_config(config: JunitaConfig) -> Result<Self> {
        ...
        text_ctx.preload_generic_styles(junita_gpu::GenericFont::SansSerif, &[400, 700], true);
        text_ctx.preload_generic_styles(junita_gpu::GenericFont::Monospace, &[400, 700], false);
    ...
    pub fn config(&self) -> &JunitaConfig {
        &self.config
    }
    ...
    /// Create a new Junita application with a window surface
    ///
    ...
    ///
    /// ```ignore
    /// let (app, surface) = JunitaApp::with_window(window_arc, None)?;
    /// ```
    #[cfg(feature = "windowed")]
    pub fn with_window<W>(
        window: Arc<W>,
        config: Option<JunitaConfig>,
