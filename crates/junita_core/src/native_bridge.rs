...
//!
//! ```ignore
//! use junita_core::native_bridge::{native_call, native_register, NativeValue};
//!
...
//! **Kotlin (Android):**
//! ```kotlin
//! JunitaNativeBridge.register("device", "get_battery_level") {
...
//! **Swift (iOS):**
//! ```swift
//! JunitaNativeBridge.shared.register(namespace: "device", name: "get_battery_level") {
...
impl NativeBridgeState {
    ...
    ///
    /// Call once at app startup (typically in JunitaContextState::init).
    ///
    /// # Panics
    ///
...
///
/// ```ignore
/// use junita_core::native_bridge::{native_register, NativeValue, NativeResult};
///
/// native_register("math", "add", |args| {
///     let a = args.get(0).and_then(|v| v.as_i32()).unwrap_or(0);
///     let b = args.get(1).and_then(|v| v.as_i32()).unwrap_or(0);
