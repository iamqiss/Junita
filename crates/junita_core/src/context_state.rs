...
//!
//! JunitaContextState provides a global singleton for accessing reactive state management
...
//! ```ignore
//! // In WindowedApp::run()
//! JunitaContextState::init(reactive, hooks, dirty_flag);
//! ```
//!
//! # Usage
//!
...
//!
//! ```ignore
//! use junita_core::context_state::{use_state_keyed, use_signal_keyed};
//!
//! // In a component:
...
static CONTEXT_STATE: OnceLock<JunitaContextState> = OnceLock::new();
...
/// Used by JunitaContextState to avoid circular dependencies with junita_layout
...
// =========================================================================
// Recorder Callbacks (for junita_recorder integration)
// =========================================================================
...
/// This allows junita_core to store the registry without depending on junita_layout
...
pub struct JunitaContextState {
    ...
    /// Stored as `AnyElementRegistry` to avoid circular dependency with junita_layout
...
impl JunitaContextState {
    ...
    pub fn init(reactive: SharedReactiveGraph, hooks: SharedHookState, dirty_flag: DirtyFlag) {
        ...
        if CONTEXT_STATE.set(state).is_err() {
            panic!("JunitaContextState::init() called more than once");
        }
    }
    ...
    pub fn get() -> &'static JunitaContextState {
        CONTEXT_STATE.get().expect(
            "JunitaContextState not initialized. Call JunitaContextState::init() at app startup.",
        )
    }

    /// Try to get the global context state (returns None if not initialized)
    pub fn try_get() -> Option<&'static JunitaContextState> {
        CONTEXT_STATE.get()
    }
    ...
    ///
    /// ```ignore
    /// use junita_core::context_state::query;
    ///
    ...
    /// circular dependencies with junita_layout.
    ...
    /// `ElementRegistry` type in junita_layout.
    ...
    ///
    /// This is a convenience method for use by junita_layout's query function.
    ...
    ///
    /// ```ignore
    /// use junita_core::context_state::query_motion;
    ///
    ...
    // =========================================================================
    // Recorder Integration (for junita_recorder)
    // =========================================================================

    /// Set the recorder event callback
    ///
    /// Called by `junita_recorder` to capture user interaction events.
    ...
    ///
    /// Called by `junita_recorder` to capture tree snapshots after each frame.
    ...
    ///
    /// Called by `junita_recorder` to track element update categories.
    ...
    // =========================================================================
    // Scroll Ref Support (for junita_layout integration)
    // =========================================================================

    /// Get or create a persisted value for scroll ref inner state
    ///
    /// This is a low-level method used by junita_layout's `use_scroll_ref` function
...
/// Create a persistent state value that survives across UI rebuilds (keyed)
///
/// This is a convenience wrapper around `JunitaContextState::get().use_state_keyed()`.
///
/// # Panics
///
/// Panics if `JunitaContextState::init()` has not been called.
///
/// # Example
///
/// ```ignore
/// use junita_core::context_state::use_state_keyed;
///
/// // In a component:
/// let open_state = use_state_keyed("my_component_open", || false);
/// ```
...
where
    ...
    F: FnOnce() -> T,
{
    JunitaContextState::get().use_state_keyed(key, init)
}

/// Create a persistent signal that survives across UI rebuilds (keyed)
///
/// This is a convenience wrapper around `JunitaContextState::get().use_signal_keyed()`.
///
/// # Panics
///
/// Panics if `JunitaContextState::init()` has not been called.
...
where
    ...
    F: FnOnce() -> T,
{
    JunitaContextState::get().use_signal_keyed(key, init)
}

/// Request a UI rebuild
///
/// This is a convenience wrapper around `JunitaContextState::get().request_rebuild()`.
///
/// # Panics
///
/// Panics if `JunitaContextState::init()` has not been called.
...
///
/// Returns the raw node ID (u64) if an element with the given ID exists.
/// This is a convenience wrapper around `JunitaContextState::get().query()`.
///
/// # Panics
///
/// Panics if `JunitaContextState::init()` has not been called.
...
pub fn query(id: &str) -> Option<u64> {
    JunitaContextState::get().query(id)
}
...
pub fn query_motion(key: &str) -> MotionAnimationState {
    JunitaContextState::get().query_motion(key)
}
