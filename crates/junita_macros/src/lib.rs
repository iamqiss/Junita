//! Junita procedural macros
//!
//! Provides derive macros for the Junita UI framework.
//!
//! # Platform-Agnostic Design
//!
//! The generated code uses traits from `junita_core` and `junita_animation`
//! rather than concrete types, enabling components to work across different
//! platforms without depending on `junita_app`.
//!
//! # Scope and Instance Awareness
//!
//! The `JunitaComponent` macro generates unique component keys that include
...
/// # Platform-Agnostic
///
/// The generated methods are generic over `JunitaContext` and `AnimationContext` traits,
/// allowing components to work with any platform implementation (desktop, Android, web, etc.)
/// without depending on `junita_app`.
///
/// # Scope and Instance Awareness
///
...
/// }
///
/// fn build_list<C: JunitaContext + AnimationContext>(ctx: &C, items: &[Item]) -> impl ElementBuilder {
...
#[proc_macro_derive(JunitaComponent, attributes(animation))]
pub fn derive_junita_component(input: TokenStream) -> TokenStream {
                    ...
                    .map(|field| {
                        ...
                        if has_animation_attr(field) {
                            ...
                            quote! {
                                ...
                                pub fn #method_name<C: junita_animation::AnimationContext>(
                                    ctx: &C,
                                    initial: f32,
                                    config: junita_animation::SpringConfig,
                                ) -> junita_animation::SharedAnimatedValue {
                                ...
                                /// }
                                /// ```
                                pub fn #method_name_for<C: junita_animation::AnimationContext, K: std::fmt::Display>(
                                    ctx: &C,
                                    instance_key: K,
                                    initial: f32,
                                    config: junita_animation::SpringConfig,
                                ...
                                #[track_caller]
                                pub fn #method_name_auto<C: junita_animation::AnimationContext>(
                                    ctx: &C,
                                    initial: f32,
                                    config: junita_animation::SpringConfig,
                                ) -> junita_animation::SharedAnimatedValue {
                            ...
                            quote! {
                                ...
                                /// Use `use_<field>_for` when you need multiple instances.
                                pub fn #method_name<C: junita_core::JunitaContext>(
                                    ctx: &C,
                                    initial: #field_type,
                                ) -> junita_core::State<#field_type>
                                where
                                ...
                                /// }
                                /// ```
                                pub fn #method_name_for<C: junita_core::JunitaContext, K: std::fmt::Display>(
                                    ctx: &C,
                                    instance_key: K,
                                    initial: #field_type,
                                ) -> junita_core::State<#field_type>
                                ...
                                /// Prefer `use_<field>_for` in loops where you control the key.
                                #[track_caller]
                                pub fn #method_name_auto<C: junita_core::JunitaContext>(
                                    ctx: &C,
                                    initial: #field_type,
                                ) -> junita_core::State<#field_type>
                                where
    ...
    let expanded = quote! {
        impl #name {
            ...
            /// key for each instantiation point. This is useful for `junita_cn` components
            ...
            pub fn use_animated_value<C: junita_animation::AnimationContext>(
                ctx: &C,
                initial: f32,
                config: junita_animation::SpringConfig,
            ) -> junita_animation::SharedAnimatedValue {
            ...
            /// ```
            pub fn use_animated_value_with<C: junita_animation::AnimationContext>(
                ctx: &C,
                suffix: &str,
                initial: f32,
                config: junita_animation::SpringConfig,
            ...
            /// Generic over any context implementing `AnimationContext`.
            pub fn use_animated_timeline<C: junita_animation::AnimationContext>(
                ctx: &C,
            ) -> junita_animation::SharedAnimatedTimeline {
            ...
            /// Generic over any context implementing `AnimationContext`.
            pub fn use_animated_timeline_with<C: junita_animation::AnimationContext>(
                ctx: &C,
                suffix: &str,
            ) -> junita_animation::SharedAnimatedTimeline {
                let key = format!("{}:{}", Self::COMPONENT_KEY, suffix);
