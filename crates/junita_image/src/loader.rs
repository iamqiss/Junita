...
impl ImageData {
    ...
    pub fn load(source: ImageSource) -> Result<Self> {
        match source {
            ImageSource::File(path) => {
                ...
                {
                    if let Some(loader) = junita_platform::assets::global_asset_loader() {
                        let asset_path =
                            junita_platform::AssetPath::from(path.to_string_lossy().to_string());
    ...
    pub fn load_asset(path: impl Into<String>) -> Result<Self> {
        ...
        let asset_path = junita_platform::AssetPath::from(path_str.clone());

        let loader = junita_platform::assets::global_asset_loader()
