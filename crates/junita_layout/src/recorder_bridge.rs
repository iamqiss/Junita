...
//!
//! This module provides helpers for sending event data to junita_recorder
//! via JunitaContextState callbacks, avoiding circular dependencies.
...
///
/// This is a no-op if JunitaContextState is not initialized or no recorder callback is set.
...
pub fn record_update(element_id: &str, category: junita_core::UpdateCategory) {
    if let Some(ctx) = JunitaContextState::try_get() {
...
) -> junita_core::UpdateCategory {
    if change.children {
        junita_core::UpdateCategory::Structural
    } else if change.layout {
        junita_core::UpdateCategory::Layout
    } else {
        junita_core::UpdateCategory::Visual
    }
}
    ...
    let visual_props = render_node.map(|n| {
        ...
        SnapshotVisualProps {
            background_color: props.background.as_ref().and_then(|brush| {
                if let junita_core::Brush::Solid(c) = brush {
                    Some(c.to_array())
                } else {
                    None
                }
