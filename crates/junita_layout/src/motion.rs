...
pub fn is_inside_animating_motion() -> bool {
    if let Some(key) = current_motion_key() {
        ...
        let state = junita_core::query_motion(&key);
        state.is_animating()
    } else {
        false
    }
...
use junita_animation::{AnimatedValue, AnimationPreset, MultiKeyframeAnimation};
...
pub struct TimelineRotation {
    ...
    pub timeline: junita_animation::SharedAnimatedTimeline,
    /// The entry ID for the rotation value in the timeline
    pub entry_id: junita_animation::TimelineEntryId,
}
...
fn motion_keyframe_to_properties(kf: &MotionKeyframe) -> junita_animation::KeyframeProperties {
    let mut props = junita_animation::KeyframeProperties::default();
...
impl Motion {
    ...
    pub fn animation(self, config: MotionAnimation) -> Self {
        use junita_animation::{Easing, KeyframeProperties};
    ...
    pub fn keyframes_from_stylesheet(
        ...
        animation_name: &str,
        duration_ms: u32,
        easing: junita_animation::Easing,
    ) -> Self {
        if let Some(keyframes) = stylesheet.get_keyframes(animation_name) {
    ...
    pub fn rotate_timeline(
        mut self,
        timeline: junita_animation::SharedAnimatedTimeline,
        entry_id: junita_animation::TimelineEntryId,
    ) -> Self {
...
/// State tracked for MotionPresence via junita_store
#[derive(Clone, Default)]
...
pub fn motion_presence_store() -> &'static junita_core::Store<MotionPresenceStore> {
    junita_core::create_store::<MotionPresenceStore>("motion_presence")
}
