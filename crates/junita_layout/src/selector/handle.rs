...
use junita_core::context_state::MotionAnimationState;
...
impl<T> ElementHandle<T> {
    ...
    pub fn is_visible(&self) -> bool {
        ...
        // Get viewport size from JunitaContextState
    ...
    pub fn scroll_into_view_with(&self, _options: ScrollOptions) {
        // Use JunitaContextState callback to scroll the element
        if let Some(ctx) = JunitaContextState::try_get() {
    ...
    pub fn emit_signal(&self, signal_id: junita_core::SignalId) {
        if let Some(ctx) = JunitaContextState::try_get() {
...
impl MotionHandle {
    ...
    ///
    /// Queries the current animation state via `JunitaContextState`.
    pub fn new(key: impl Into<String>) -> Self {
        let key = key.into();
        let state = JunitaContextState::try_get()
