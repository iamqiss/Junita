...
/// access to `WindowedContext`. It uses `JunitaContextState` for persistence
/// across rebuilds.
///
/// # Example
///
/// ```rust,ignore
/// use junita_layout::selector::use_scroll_ref;
///
...
/// # Panics
///
/// Panics if `JunitaContextState` has not been initialized.
pub fn use_scroll_ref(key: &str) -> ScrollRef {
    ...
    // Get or create the inner state using JunitaContextState's persisted storage
