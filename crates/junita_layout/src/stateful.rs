...
impl TimelineHandle {
    ...
    pub fn get(&self, entry_id: junita_animation::TimelineEntryId) -> Option<f32> {
...
/// animation that pulls content back to bounds. Uses `junita_animation::Spring`
...
///
/// This bridges `JunitaContextState` (which stores arbitrary values via signals)
...
{
    use junita_core::context_state::JunitaContextState;

    let ctx = JunitaContextState::get();

    // We store the SharedState wrapped in an Option inside the signal
    // This way it persists across rebuilds
    let state: junita_core::State<Option<SharedState<S>>> = ctx.use_state_keyed(key, || None);
...
pub struct StateContext<S: StateTransitions> {
    ...
    reactive: junita_core::context_state::SharedReactiveGraph,
...
impl<S: StateTransitions> StateContext<S> {
    ...
    /// ```
    pub fn use_signal<T, F>(&self, name: &str, init: F) -> junita_core::State<T>
    where
        T: Clone + Send + 'static,
        F: FnOnce() -> T,
    {
        let signal_key = format!("{}:signal:{}", self.full_key(), name);
        let state = junita_core::context_state::use_state_keyed(&signal_key, init);
    ...
    pub(crate) fn reactive(&self) -> &junita_core::context_state::SharedReactiveGraph {
        &self.reactive
    }
    ...
    /// `junita_animation::set_global_scheduler()` at app startup).
    ///
    /// # Example
    ///
    /// ```ignore
    ...
    pub fn use_animated_value_with_config(
        ...
        let handle = junita_animation::get_scheduler();
    ...
    where
        ...
        T: junita_animation::ConfigureResult,
    {
    ...
    pub fn dep<T: Clone + Send + Default + 'static>(&self, index: usize) -> Option<T> {
        ...
        let signal = junita_core::Signal::from_id(*signal_id);
    ...
    pub fn dep_as_state<T: Clone + Send + 'static>(
        &self,
        index: usize,
    ) -> Option<junita_core::State<T>> {
        let signal_id = self.deps.get(index)?;
        let signal = junita_core::Signal::from_id(*signal_id);
        let dirty_flag = junita_core::context_state::JunitaContextState::get()
            .dirty_flag()
            .clone();
        Some(junita_core::State::new(
            signal,
            self.reactive.clone(),
            dirty_flag,
        ))
    ...
    ///     })
    /// ```
    pub fn dep_signal_id(&self, index: usize) -> Option<junita_core::SignalId> {
    ...
    ///     })
    /// ```
    pub fn use_effect<T: Clone + Send + 'static>(&self, signal: &junita_core::State<T>) {
...
pub struct StatefulBuilder<S: StateTransitions> {
    ...
    deps: Vec<junita_core::reactive::SignalId>,
...
impl<S: StateTransitions + Default> StatefulBuilder<S> {
    ...
    pub fn deps(mut self, deps: impl IntoIterator<Item = junita_core::reactive::SignalId>) -> Self {
        ...
        F: Fn(&StateContext<S>) -> crate::div::Div + Send + Sync + 'static,
    {
        ...
        let reactive = junita_core::context_state::JunitaContextState::get()
            .reactive()
            .clone();
...
impl<S: StateTransitions> Stateful<S> {
    ...
    fn handle_event_internal(
        ...
        if let Some(ref motion_key) = guard.ancestor_motion_key {
            let motion_state = junita_core::query_motion(motion_key);
    ...
    pub fn bg(self, color: impl Into<junita_core::Brush>) -> Self {
    ...
    pub fn border(self, width: f32, color: junita_core::Color) -> Self {
        self.merge_into_inner(Div::new().border(width, color));
        self
    }

    /// Set border color only (builder pattern)
    pub fn border_color(self, color: junita_core::Color) -> Self {
    ...
    pub fn shadow(self, shadow: junita_core::Shadow) -> Self {
    ...
    pub fn transform(self, transform: junita_core::Transform) -> Self {
        ...
        F: Fn(&crate::event_handler::EventContext) + Send + Sync + 'static,
    {
        self.event_handlers_cache
            .borrow_mut()
            .on(junita_core::events::event_types::POINTER_MOVE, handler);
        self
    }
        ...
        F: Fn(&crate::event_handler::EventContext) + Send + Sync + 'static,
    {
        self.event_handlers_cache
            .borrow_mut()
            .on(junita_core::events::event_types::DRAG, handler);
        self
    }
        ...
        F: Fn(&crate::event_handler::EventContext) + Send + Sync + 'static,
    {
        self.event_handlers_cache
            .borrow_mut()
            .on(junita_core::events::event_types::DRAG_END, handler);
        self
    }
    ...
    pub fn on_event<F>(self, event_type: junita_core::events::EventType, handler: F) -> Self
    where
        F: Fn(&crate::event_handler::EventContext) + Send + Sync + 'static,
    {
        self.event_handlers_cache
...
impl<S: StateTransitions> BoundStateful<S> {
    ...
    /// Set transform (builder pattern)
    pub fn transform_style(self, xform: junita_core::Transform) -> Self {
...
mod tests {
    use super::*;
    use junita_core::events::event_types;
    use junita_core::{Brush, Color, CornerRadius};
