...
//!
//! ```ignore
//! use junita_gpu::GpuPaintContext;
//! use junita_core::{DrawContext, DrawContextExt, Color, Rect};
//!
        ...
        match shape {
            ...
            ClipShape::Circle { center, radius } => {
                ...
                if (scale_x - scale_y).abs() < 0.001 {
                    ...
                    ClipShape::Ellipse {
                        center: transformed_center,
                        radii: junita_core::Vec2::new(radius * scale_x, radius * scale_y),
                    }
                }
            }
            ClipShape::Ellipse { center, radii } => {
                ...
                ClipShape::Ellipse {
                    center: transformed_center,
                    radii: junita_core::Vec2::new(radii.x * scale_x, radii.y * scale_y),
                }
            }
            ClipShape::Path(path) => {
        ...
        match brush {
            ...
            Brush::Gradient(gradient) => {
                let (stops, fill_type, gradient_params) = match gradient {
                    ...
                    junita_core::Gradient::Conic { center, stops, .. } => (
                        stops,
                        FillType::RadialGradient,
...
impl<'a> DrawContext for GpuPaintContext<'a> {
    ...
    fn draw_text(&mut self, text: &str, origin: Point, style: &TextStyle) {
        use junita_core::{TextAlign, TextBaseline};
        use junita_text::{TextAlignment, TextAnchor};
...
impl<'a, 'b> SdfBuilder for GpuSdfBuilder<'a, 'b> {
    ...
    fn ellipse(&mut self, center: Point, radii: junita_core::Vec2) -> ShapeId {
...
mod tests {
    ...
    fn test_execute_commands() {
        use junita_core::RecordingContext;
