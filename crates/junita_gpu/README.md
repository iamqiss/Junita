...
GPU renderer for Junita UI using [wgpu](https://wgpu.rs/) with SDF-based rendering.

## Overview

`junita_gpu` provides high-performance GPU rendering for the Junita UI framework. It uses Signed Distance Field (SDF) techniques for crisp, resolution-independent rendering of UI primitives.
...
- `JUNITA_WGPU_MAX_BUFFER_MB`: Override wgpu `max_buffer_size` (MiB), clamped to adapter-supported max.
- `JUNITA_GPU_MAX_PRIMITIVES`: Override max SDF primitives per batch (clamped by storage buffer binding size).
- `JUNITA_GPU_MAX_GLYPHS`: Override max glyphs per batch (clamped by storage buffer binding size).
- `JUNITA_GPU_MAX_GLASS_PRIMITIVES`: Override max glass primitives per batch (clamped by storage buffer binding size).
...
```rust
use junita_gpu::{GpuRenderer, GpuPrimitive};
