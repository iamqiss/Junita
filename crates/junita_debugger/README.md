...
Visual debugger application for Junita UI recordings.

## Overview

`junita_debugger` is a standalone GUI application for inspecting recorded Junita UI sessions. It provides visual tools for debugging layout issues, tracking events, and analyzing UI state.
...
```bash
cargo install junita_debugger
```

Or build from source:

```bash
cargo build -p junita_debugger --release
```
...
junita-debugger recording.json
...
The debugger reads JSON files created by `junita_recorder`:
