...
use junita_cn::components::button::{button, ButtonSize, ButtonVariant};
use junita_cn::components::select::{select, SelectSize};
use junita_cn::components::separator::separator;
use junita_cn::components::slider::{slider, SliderSize};
use junita_core::context_state::JunitaContextState;
use junita_core::Color;
use junita_icons::icons;
use junita_layout::div::{Div, ElementBuilder};
use junita_layout::element::RenderProps;
use junita_layout::event_handler::EventHandlers;
use junita_layout::prelude::*;
use junita_layout::tree::{LayoutNodeId, LayoutTree};
use junita_recorder::replay::ReplayState;
use junita_recorder::{RecordedEvent, Timestamp, TimestampedEvent};
...
impl BuiltTimelinePanel {
    ...
    fn controls(config: &TimelinePanelConfig) -> Div {
        ...
        let speed_state =
            JunitaContextState::get().use_state_keyed("timeline_speed", || speed_str.clone());
    ...
    fn timeline_track(config: &TimelinePanelConfig) -> Div {
        ...
        let position_state =
            JunitaContextState::get().use_state_keyed("timeline_position", || position_norm);
