...
High-quality text rendering for Junita UI.

## Overview

`junita_text` provides comprehensive text rendering capabilities including font loading, text shaping, glyph rasterization, and layout.
...
```rust
use junita_text::{global_font_registry, TextRenderer, PreparedText};
...
```rust
use junita_text::global_font_registry;
...
// Load system fonts
for path in junita_app::system_font_paths() {
...
```rust
use junita_text::{TextShaper, ShapedText};
...
```rust
use junita_text::{TextLayout, TextLayoutEngine, TextAlign};
...
```rust
use junita_text::{EmojiRenderer, is_emoji, contains_emoji};
