...
private let log = OSLog(subsystem: "com.junita.{{project_name_snake}}", category: "JunitaViewController")

/// Main view controller for Junita iOS applications
///
/// This controller manages the Metal rendering surface, display link for
/// frame timing, and touch event routing to the Junita framework.
///
/// Usage:
...
class JunitaViewController: UIViewController {
    ...
    /// Junita render context (manages UI state)
    private var renderContext: OpaquePointer?

    /// Junita GPU renderer (manages Metal rendering)
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let gpu = gpuRenderer {
            junita_gpu_resize(gpu, width, height)
        }
    }
    ...
    deinit {
        ...
        // Clean up Junita resources
        if let gpu = gpuRenderer {
            junita_destroy_gpu(gpu)
        }
        if let ctx = renderContext {
    ...
    private func initializeJunita() {
        ...
        os_log(.info, log: log, "Calling junita_create_context()")
        guard let ctx = junita_create_context(width, height, Double(scale)) else {
            os_log(.error, log: log, "Failed to create Junita render context")
            return
        }
        renderContext = ctx
        ...
        os_log(.info, log: log, "Calling junita_init_gpu()")

        guard let gpu = junita_init_gpu(ctx, layerPtr, width, height) else {
            os_log(.error, log: log, "Failed to initialize Junita GPU renderer")
            return
        }
        gpuRenderer = gpu
        ...
        os_log(.info, log: log, "Junita fully initialized: %dx%d @ %.1fx", width, height, scale)
    }
    ...
    private func loadBundledFonts(gpu: OpaquePointer) {
        ...
        for fontName in fontNames {
            if let fontPath = Bundle.main.path(forResource: fontName.replacingOccurrences(of: ".ttf", with: ""),
                ...
                let loaded = junita_load_bundled_font(gpu, fontPath)
    ...
    @objc private func displayLinkFired() {
        ...
        let needsRender = junita_needs_render(ctx)
        if frameCount < 3 {
        ...
        if frameCount < 3 {
            os_log(.debug, log: log, "Frame %d - calling junita_build_frame", frameCount)
        }
        junita_build_frame(ctx)

        // Render to Metal
        if frameCount < 3 {
            os_log(.debug, log: log, "Frame %d - calling junita_render_frame", frameCount)
        }
        let result = junita_render_frame(gpu)
        if frameCount < 3 {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            os_log(.info, log: log, "touchesBegan: calling junita_handle_touch at (%.1f, %.1f)", point.x, point.y)
            junita_handle_touch(ctx, touchId, Float(point.x), Float(point.y), 0) // 0 = began
        }
    }
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            let point = touch.location(in: view)
            let touchId = getTouchId(for: touch)
            junita_handle_touch(ctx, touchId, Float(point.x), Float(point.y), 1) // 1 = moved
        }
    }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            let point = touch.location(in: view)
            let touchId = getTouchId(for: touch)
            os_log(.info, log: log, "touchesEnded: calling junita_handle_touch at (%.1f, %.1f)", point.x, point.y)
            junita_handle_touch(ctx, touchId, Float(point.x), Float(point.y), 2) // 2 = ended
    ...
    override func touchesCancelled(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            let point = touch.location(in: view)
            let touchId = getTouchId(for: touch)
            junita_handle_touch(ctx, touchId, Float(point.x), Float(point.y), 3) // 3 = cancelled
            removeTouchId(for: touch)
        }
    }
...
///
/// fn main() {
///     junita_set_ui_builder(my_ui_builder);
/// }
/// ```
