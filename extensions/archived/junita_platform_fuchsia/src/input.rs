...
//!
//! Converts fuchsia.ui.pointer and fuchsia.ui.input3 events to Junita input events.
//!
//! # Input Sources
//!
...
/// Convert a Fuchsia touch to a Junita input event
...
/// Convert fuchsia.ui.pointer.TouchEvent to Junita Touch
#[cfg(all(target_os = "fuchsia", feature = "fuchsia-sdk"))]
...
impl FuchsiaMouseButton {
    ...
    /// Convert to Junita MouseButton
    pub fn to_junita(self) -> MouseButton {
        match self.0 {
            0 => MouseButton::Left,
            1 => MouseButton::Right,
...
impl Mouse {
    ...
    pub fn get_button(&self) -> Option<MouseButton> {
        self.button.map(|b| b.to_junita())
    }
}

/// Convert a Fuchsia mouse event to a Junita input event
...
/// Convert a Fuchsia key event to a Junita input event
pub fn convert_key(key_event: &KeyEvent) -> InputEvent {
    ...
    InputEvent::Keyboard(KeyboardEvent {
        key: fuchsia_key_to_junita(key_event.key),
...
/// Convert Fuchsia key code to Junita Key
fn fuchsia_key_to_junita(key: u32) -> Key {
    use key_codes::*;
