//! Junita Android Platform
//!
//! Android platform implementation for Junita UI framework.
//!
//! This crate implements the `junita_platform` traits for Android,
...
//! use junita_platform_android::AndroidPlatform;
//!
...
use junita_platform::{Platform, PlatformError};
...
pub mod input_convert {
    ...
    use junita_platform::{InputEvent, TouchEvent as JunitaTouchEvent};

    /// Convert Android TouchEvent to junita_platform InputEvent
    pub fn convert_touch_event(event: &TouchEvent) -> InputEvent {
        match event {
            TouchEvent::Down { pointer, .. } => InputEvent::Touch(JunitaTouchEvent::Started {
            ...
            TouchEvent::Move { pointers } => {
                ...
                if let Some(p) = pointers.first() {
                    InputEvent::Touch(JunitaTouchEvent::Moved {
                        id: p.id as u64,
                        x: p.x,
                        y: p.y,
                        pressure: p.pressure,
                    ...
                    InputEvent::Touch(JunitaTouchEvent::Cancelled { id: 0 })
                }
            }
            TouchEvent::Up { pointer, .. } => InputEvent::Touch(JunitaTouchEvent::Ended {
                id: pointer.id as u64,
                x: pointer.x,
                y: pointer.y,
            }),
            TouchEvent::Cancel => InputEvent::Touch(JunitaTouchEvent::Cancelled { id: 0 }),
        }
    }
