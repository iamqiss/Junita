...
#[cfg(target_os = "android")]
impl JunitaAndroidApp {
    /// Create a new Junita Android application
    pub fn new() -> Self {
        Self {
            window: None,
            running: true,
    ...
    fn init_graphics(&mut self) {
        if let Some(ref _window) = self.window {
            ...
            // This will use junita_gpu with Vulkan backend
...
#[cfg(target_os = "android")]
impl Default for JunitaAndroidApp {
    fn default() -> Self {
        Self::new()
    }
}
...
/// junita_app::AndroidApp::run() instead.
...
pub fn android_main(app: AndroidApp) {
    ...
    let mut junita_app = JunitaAndroidApp::new();

    while junita_app.is_running() {
        // Poll for events with 16ms timeout (roughly 60fps)
        app.poll_events(Some(std::time::Duration::from_millis(16)), |event| {
            junita_app.handle_event(&app, event);
        });

        // Render if we have a window and are focused
        if junita_app.should_render() {
            junita_app.render_frame();
        }
    }

    info!("Junita Android app shutting down");
}
