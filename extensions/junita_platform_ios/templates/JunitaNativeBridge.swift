 ...
 * JunitaNativeBridge.shared.registerDefaults()
 * JunitaNativeBridge.shared.connectToRust()
 *
 * // Or register custom handlers
 * JunitaNativeBridge.shared.register(namespace: "myapi", name: "my_function") { args in
 *     // args is [Any]
 *     return "result"
 * }
 * ```
...
/// Returns a malloc'd string that Rust must free with junita_free_string
@_cdecl("junita_ios_native_call")
public func junita_ios_native_call(
...
/// Free a string allocated by junita_ios_native_call
@_cdecl("junita_free_string")
public func junita_free_string(ptr: UnsafeMutablePointer<CChar>?) {
    if let ptr = ptr {
        free(ptr)
    }
}
