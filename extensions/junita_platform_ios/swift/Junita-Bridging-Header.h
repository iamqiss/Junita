...
//  Junita iOS Bridge
//
//  C declarations for Swift interop with Junita Rust library.
...
///
/// Call this each frame when junita_needs_render() returns true.
...
/// @return JSON-encoded result string (caller must free with junita_free_string)
...
/// @param call_fn Function pointer to Swift's junita_ios_native_call
void junita_set_native_call_fn(NativeCallFn call_fn);

/// Check if native bridge is ready
/// @return true if native call function has been registered
bool junita_native_bridge_is_ready(void);
