...
//!
//! This crate implements the `junita_platform` traits for iOS,
//! providing touch input, lifecycle management, and window handling
//! via UIKit and Metal.
//!
//! # Architecture
//!
//! Unlike desktop platforms where Junita owns the event loop, on iOS
//! the event loop is managed by UIKit's RunLoop. Junita integrates with
//! iOS through:
//!
...
pub use native_bridge::{
    junita_native_bridge_is_ready, junita_set_native_call_fn, IOSNativeBridgeAdapter,
};
...
use junita_platform::PlatformError;
